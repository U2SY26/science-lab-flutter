...
class _PrisonersDilemmaScreenPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int r = 0; r < 2; r++) {
      for (int c = 0; c < 2; c++) {
        ...
        canvas.drawRect(
          Rect.fromLTWH(rx, ry, cellS, cellS),
          Paint()
            ..color = (isNash ? const Color(0xFF64FF8C) : payColors[r][c])
    ...
    _text(
      canvas,
      'C  D',
      Offset(boxX + cellS * 0.25, boxY - 10),
      const TextStyle(color: Color(0xFF00D4FF), fontSize: 7),
    );
    _text(
      canvas,
      'C\nD',
      Offset(boxX - 10, boxY + cellS * 0.2),
    ...
    _text(
      canvas,
      'P1: ${stratNames[strat]}',
      Offset(boxX + cellS * 2 + 8, boxY + 4),
      TextStyle(
        color: stratColors[strat],
        fontSize: 9,
        fontWeight: FontWeight.bold,
      ),
    );
    ...
    canvas.drawLine(
      Offset(padL, graphY + graphH),
      Offset(padL + graphW, graphY + graphH),
      Paint()
        ..color = const Color(0xFF5A8A9A)
        ..strokeWidth = 1,
    );
    ...
    for (int s = 0; s < 4; s++) {
      ...
      canvas.drawRRect(
        RRect.fromRectAndRadius(
          Rect.fromLTWH(padL, by, bw, barH),
          const Radius.circular(2),
        ),
        Paint()
          ..color = stratColors[s].withValues(alpha: s == strat ? 0.85 : 0.35),
      );
      _text(
        canvas,
        '${stratNames[s]}: ${scores[s].toInt()}',
        Offset(padL + bw + 4, by + 1),
        TextStyle(
          color: stratColors[s].withValues(alpha: s == strat ? 1.0 : 0.5),
          fontSize: 7,
        ),
      );
    }
  ...
  @override
  bool shouldRepaint(covariant _PrisonersDilemmaScreenPainter oldDelegate) =>
      true;
}
