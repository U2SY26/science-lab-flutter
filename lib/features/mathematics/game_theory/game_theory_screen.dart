...
class _GameTheoryScreenState extends State<GameTheoryScreen>
  ...
  void _update() {
    ...
    setState(() {
      _time += 0.016;
      _eu1 =
          _p1Prob * (_p2Prob * 3 + (1 - _p2Prob) * 0) +
          (1 - _p1Prob) * (_p2Prob * 5 + (1 - _p2Prob) * 1);
      _eu2 =
          _p2Prob * (_p1Prob * 3 + (1 - _p1Prob) * 0) +
          (1 - _p2Prob) * (_p1Prob * 5 + (1 - _p1Prob) * 1);
    });
  }
...
class _GameTheoryScreenPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
      ...
      for (int c = 0; c < 3; c++) {
        ...
        if (r == 0 && c == 0) {
          ...
          _text(
            canvas,
            label,
            Offset(rx + cellW / 2 - 4, ry + cellH / 2 - 6),
          ...
          if (isNash) {
            bgCol = const Color(0xFF64FF8C).withValues(alpha: 0.18);
          } else if (isBrP1 && isBrP2) {
            bgCol = const Color(0xFF00D4FF).withValues(alpha: 0.08);
          }
          ...
          _text(
            ...
            '$v1',
            Offset(rx + 5, ry + 4),
            TextStyle(color: c1, fontSize: 10, fontWeight: FontWeight.bold),
          );
          _text(
            canvas,
            '$v2',
            Offset(rx + cellW - 14, ry + cellH - 16),
            TextStyle(color: c2, fontSize: 10, fontWeight: FontWeight.bold),
          );

          // Best response arrows
          if (isBrP1) {
            canvas.drawLine(
              Offset(rx + 3, ry + cellH - 5),
              Offset(rx + 3, ry + cellH - 2),
              Paint()
          ...
          if (isBrP2) {
            canvas.drawLine(
              Offset(rx + cellW - 3, ry + 3),
              Offset(rx + cellW - 6, ry + 3),
              Paint()
    ...
    _text(
      canvas,
      'P2→',
      Offset(matLeft + cellW * 0.8, matTop - 12),
      const TextStyle(color: Color(0xFF00D4FF), fontSize: 8),
    );
    _text(
      canvas,
      'P1↓',
      Offset(matLeft - 22, matTop + cellH * 0.6),
    ...
    _text(
      canvas,
      '★ 내시 균형: (D,D)=(1,1)',
      Offset(matLeft, matTop + cellH * 3 + 6),
      const TextStyle(
    ...
    // EU = p1*(p2*3+…) etc
    final eu1 =
        p1Prob * (p2Prob * 3 + (1 - p2Prob) * 0) +
        (1 - p1Prob) * (p2Prob * 5 + (1 - p2Prob) * 1);
    final eu2 =
        p2Prob * (p1Prob * 3 + (1 - p1Prob) * 0) +
        (1 - p2Prob) * (p1Prob * 5 + (1 - p1Prob) * 1);

    final infoItems = [
      (
        'p₁(C)',
        '${(p1Prob * 100).toStringAsFixed(0)}%',
        const Color(0xFFFF6B35),
      ),
      (
        'p₂(C)',
        '${(p2Prob * 100).toStringAsFixed(0)}%',
        const Color(0xFF00D4FF),
      ),
    ...
    _text(
      canvas,
      'p₂(C)',
      Offset(brX + brW / 2 - 8, brY + brH + 2),
      const TextStyle(color: Color(0xFF5A8A9A), fontSize: 7),
    );
    _text(
      canvas,
      'p₁',
      Offset(brX - 12, brY + brH / 2 - 4),
      const TextStyle(color: Color(0xFF5A8A9A), fontSize: 7),
    );
  }
