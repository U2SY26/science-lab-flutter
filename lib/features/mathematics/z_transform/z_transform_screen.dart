...
class _ZTransformScreenPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (double x = 0; x < size.width; x += 28) {
      canvas.drawLine(Offset(x, padT), Offset(x, padT + planeH), gridP);
    }
    for (double y = padT; y < padT + planeH; y += 28) {
    ...
    canvas.drawRect(
      Rect.fromLTWH(0, padT, size.width, planeH),
    ...
    for (int i = 3; i >= 1; i--) {
      canvas.drawCircle(
        ...
        unitR + i * 2.5,
        Paint()
          ..color = AppColors.accent.withValues(alpha: 0.04 * i * pulse)
    ...
    final polePositions = [
      ...
      Offset(
        cx + poleR * math.cos(-angleRad),
        cy - poleR * math.sin(-angleRad),
      ),
    ];
    ...
    _drawLabel(
      canvas,
      '단위원',
      Offset(cx + unitR + 3, cy - 10),
      AppColors.accent,
      fontSize: 9,
    );
    _drawLabel(
      canvas,
      isStable ? '안정 (|z|<1)' : '불안정 (|z|>1)',
      Offset(8, padT + 4),
      isStable ? AppColors.accent : const Color(0xFFFF4444),
      fontSize: 9,
    );
    _drawLabel(
      canvas,
      'Re',
      Offset(size.width - 20, cy - 12),
      AppColors.muted,
      fontSize: 9,
    );
    _drawLabel(
    ...
    canvas.drawLine(
      Offset(stemLeft, stemCy),
      Offset(stemRight, stemCy),
      Paint()
