...
class _LaplaceTransformScreenPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final axP = Paint()
      ...
      ..strokeWidth = 0.8;
    canvas.drawLine(
      Offset(leftRect.left + 4, lCy),
      Offset(leftRect.right - 4, lCy),
      axP,
    );
    canvas.drawLine(
      Offset(leftRect.left + 20, leftRect.top + 4),
      Offset(leftRect.left + 20, leftRect.bottom - 4),
      axP,
    );

    // Scan line (moving) — animate transform "happening"
    final scanX =
        leftRect.left + 20 + (leftRect.width - 24) * ((time * 0.25) % 1.0);
    canvas.drawLine(
      Offset(scanX, leftRect.top + 4),
      Offset(scanX, leftRect.bottom - 4),
      Paint()
    ...
    // Axes
    canvas.drawLine(
      Offset(rightRect.left + 4, rCy),
      Offset(rightRect.right - 4, rCy),
      axP,
    );
    canvas.drawLine(
      Offset(rCx, rightRect.top + 4),
      Offset(rCx, rightRect.bottom - 4),
      axP,
    );

    // Grid lines on s-plane
    final sGridP = Paint()
      ..color = AppColors.simGrid.withValues(alpha: 0.25)
      ..strokeWidth = 0.5;
    for (int i = -3; i <= 3; i++) {
      if (i == 0) continue;
      ...
      canvas.drawLine(
        Offset(gx, rightRect.top + 4),
        Offset(gx, rightRect.bottom - 4),
        sGridP,
      );
      canvas.drawLine(
        Offset(rightRect.left + 4, gy),
        Offset(rightRect.right - 4, gy),
        sGridP,
      );
    }
    ...
    _drawLabel(
      canvas,
      'f(t) 시간 영역',
      Offset(leftRect.left + 4, leftRect.top + 2),
      AppColors.ink,
      fontSize: 9,
    );
    _drawLabel(
      canvas,
      's-평면 (ROC 녹색)',
      Offset(rightRect.left + 4, rightRect.top + 2),
      AppColors.ink,
      fontSize: 9,
    );
    _drawLabel(
      canvas,
      'σ',
      Offset(rightRect.right - 12, rCy - 12),
      AppColors.muted,
      fontSize: 9,
    );
    _drawLabel(
  ...
  @override
  bool shouldRepaint(covariant _LaplaceTransformScreenPainter oldDelegate) =>
      true;
}
