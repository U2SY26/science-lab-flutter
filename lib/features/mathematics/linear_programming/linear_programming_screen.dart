...
class LinearProgrammingScreen extends StatefulWidget {
  ...
  @override
  State<LinearProgrammingScreen> createState() =>
      _LinearProgrammingScreenState();
}
...
class _LinearProgrammingScreenPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i <= 5; i++) {
      canvas.drawLine(
        Offset(sx(i * domMax / 5), padT),
        Offset(sx(i * domMax / 5), padT + plotH),
        gridP,
      );
      canvas.drawLine(
        Offset(padL, sy(i * domMax / 5)),
        Offset(padL + plotW, sy(i * domMax / 5)),
        gridP,
      );
    }
    ...
    _text(
      canvas,
      'x₁',
      Offset(padL + plotW - 10, padT + plotH + 5),
    ...
    for (int i = 0; i <= 3; i++) {
      ...
      _text(
        canvas,
        val,
        Offset(sx(b * i / 3) - 4, padT + plotH + 4),
        const TextStyle(color: Color(0xFF5A8A9A), fontSize: 7),
      );
      _text(
        canvas,
        val,
        Offset(padL - 16, sy(b * i / 3) - 4),
        const TextStyle(color: Color(0xFF5A8A9A), fontSize: 7),
      );
    }
    ...
    for (int i = 0; i < vx.length; i++) {
      if (vx[i] >= -0.01 && vy[i] >= -0.01) {
        polyPts.add(
          Offset(sx(vx[i].clamp(0, domMax)), sy(vy[i].clamp(0, domMax))),
        );
      }
    }
    ...
    if (polyPts.length >= 3) {
      ...
      for (int i = 1; i < polyPts.length; i++) {
        path.lineTo(polyPts[i].dx, polyPts[i].dy);
      }
      path.close();
      canvas.drawPath(
        path,
    ...
    void drawConstraintLine(
      ...
      canvas.drawLine(
        Offset(sx(x1a), sy(y1a)),
        Offset(sx(x2a), sy(y2a)),
        Paint()
          ..color = col
          ..strokeWidth = 1.2
      ...
      _text(
        canvas,
        lbl,
        Offset(sx(x2a) + 2, sy(y2a) - 4),
        TextStyle(color: col, fontSize: 7),
      );
    }
    ...
    for (final pt in corners) {
      canvas.drawCircle(
        pt,
        3.5,
        Paint()..color = const Color(0xFF5A8A9A).withValues(alpha: 0.7),
      );
    }
    ...
    for (int k = 1; k <= 3; k++) {
      ...
      if (c2obj.abs() > 0.01) {
        ...
        canvas.drawLine(
          ...
          Paint()
            ..color = const Color(0xFFFFD700).withValues(alpha: 0.2 + k * 0.1)
            ..strokeWidth = 0.8,
        );
      }
    }
    ...
    _text(
      canvas,
      'z*=${bestZ.toStringAsFixed(1)}',
      Offset(bestPt.dx + 6, bestPt.dy - 10),
      const TextStyle(
    ...
    final optX = candidateData.reduce((a, b2) {
      return (c1 * a.$1 + c2obj * a.$2) >= (c1 * b2.$1 + c2obj * b2.$2)
          ? a
          : b2;
    });
    _text(
      canvas,
      '최적해: (x₁=${optX.$1.toStringAsFixed(1)}, x₂=${optX.$2.toStringAsFixed(1)})  z*=${bestZ.toStringAsFixed(2)}',
      Offset(padL, sumY),
      const TextStyle(color: Color(0xFF5A8A9A), fontSize: 8),
    );
    _text(
      canvas,
      '목적함수 방향: ${(objAngle * 180 / math.pi).toStringAsFixed(0)}°',
      Offset(padL, sumY + 12),
      const TextStyle(color: Color(0xFF5A8A9A), fontSize: 8),
    );
    _text(
  ...
  @override
  bool shouldRepaint(covariant _LinearProgrammingScreenPainter oldDelegate) =>
      true;
}
