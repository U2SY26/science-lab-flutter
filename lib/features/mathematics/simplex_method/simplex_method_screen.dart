      ...
      body: SingleChildScrollView(
        ...
        child: SimulationContainer(
          ...
          simulation: SizedBox(
            height: 350,
            child: CustomPaint(
              painter: _SimplexMethodScreenPainter(time: _time, step: _step),
              size: Size.infinite,
            ),
          ),
          controls: Column(
...
class _SimplexMethodScreenPainter extends CustomPainter {
  ...
  _SimplexMethodScreenPainter({required this.time, required this.step});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i <= 4; i++) {
      canvas.drawLine(
        Offset(sx(i.toDouble()), padT),
        Offset(sx(i.toDouble()), padT + areaH),
        gridP,
      );
      canvas.drawLine(
        Offset(padL, sy(i.toDouble())),
        Offset(padL + areaW, sy(i.toDouble())),
        gridP,
      );
    }
    ...
    _text(
      canvas,
      'x₁',
      Offset(padL + areaW - 8, padT + areaH + 5),
      const TextStyle(color: Color(0xFF5A8A9A), fontSize: 8),
    );
    _text(
      canvas,
      'x₂',
      Offset(padL - 14, padT - 2),
      const TextStyle(color: Color(0xFF5A8A9A), fontSize: 8),
    );
    ...
    final currentVertexIdx = currentStep < _path.length
        ? _path[currentStep]
        : _path.last;
    ...
    if (bestZ > 0) {
      ...
      canvas.drawLine(
        Offset(sx(x1a), sy(y1a.clamp(0, domMax))),
    ...
    for (int s = 0; s < currentStep && s < _path.length - 1; s++) {
      ...
      canvas.drawLine(
        Offset(sx(v1.$1), sy(v1.$2)),
        Offset(sx(v2.$1), sy(v2.$2)),
        Paint()
      ...
      canvas.drawCircle(
        ...
        Paint()
          ..color = isActive
              ...
              : const Color(0xFF5A8A9A).withValues(alpha: 0.6),
      );
      _text(
        canvas,
        'V$i',
        Offset(sx(v.$1) + 5, sy(v.$2) - 10),
        TextStyle(
          color: isActive ? const Color(0xFFFFD700) : const Color(0xFF5A8A9A),
          fontSize: 7,
        ),
      );
    }
    ...
    for (int c = 0; c < 4; c++) {
      canvas.drawRect(
        Rect.fromLTWH(tableX + c * colW, tableY + 4, colW, rowH),
        Paint()..color = const Color(0xFF1A3040),
      );
      _text(
        canvas,
        headers[c],
        Offset(tableX + c * colW + 3, tableY + 7),
        const TextStyle(
          color: Color(0xFF5A8A9A),
          fontSize: 7,
          fontWeight: FontWeight.bold,
    ...
    for (int r = 0; r < tData.length; r++) {
      ...
      for (int c = 0; c < 4; c++) {
        ...
        _text(
          canvas,
          row[c],
          Offset(tableX + c * colW + 3, tableY + 8 + (r + 1) * rowH),
          TextStyle(
            color: isPivot ? const Color(0xFF00D4FF) : const Color(0xFFE0F4FF),
            fontSize: 7,
          ),
        );
      }
    ...
    canvas.drawLine(
      Offset(tableX, chartY + chartH),
      Offset(tableX + chartW, chartY + chartH),
      Paint()
        ..color = const Color(0xFF1A3040)
        ..strokeWidth = 0.8,
    );
    ...
    for (int s = 0; s <= currentStep && s < _path.length; s++) {
      ...
      if (firstZ) {
        convPath.moveTo(gx, gy);
        firstZ = false;
      } else {
        convPath.lineTo(gx, gy);
      }
      canvas.drawCircle(
        Offset(gx, gy),
        s == currentStep ? 4.5 : 2.5,
        Paint()
          ..color = s == currentStep
              ? const Color(0xFFFFD700)
              : const Color(0xFF64FF8C),
    ...
    // Bottom status
    final status = currentStep >= _path.length - 1
        ? '최적 (reduced cost ≥ 0)'
        : '피벗 진행 중...';
    _text(
      canvas,
      ...
      Offset(padL, h - 12),
      TextStyle(
        color: currentStep >= _path.length - 1
            ? const Color(0xFF64FF8C)
