...
class _BinaryStarScreenState extends State<BinaryStarScreen>
  ...
  void _update() {
    ...
    setState(() {
      _time += 0.016;
      _period =
          math.pow(_separation, 1.5).toDouble() / math.sqrt(1 + _massRatio);
      _v1 =
          2 * math.pi * _separation * _massRatio / (_period * (1 + _massRatio));
      _v2 = _v1 / _massRatio;
    });
  }
...
class _BinaryStarScreenPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Orbital scale
    final scaleAU =
        math.min(size.width, orbitH) * 0.32 / separationDist.clamp(0.1, 10.0);
    ...
    for (int i = 1; i <= 12; i++) {
      ...
      canvas.drawCircle(
        Offset(tx1, ty1),
        1.5,
        Paint()..color = const Color(0xFF00D4FF).withValues(alpha: alpha),
      );
      canvas.drawCircle(
        Offset(tx2, ty2),
        1.5,
        Paint()..color = const Color(0xFFFF6B35).withValues(alpha: alpha),
      );
    }
    ...
    _drawLabel(
      canvas,
      dopplerStr1,
      Offset(x1 - 20, y1 + r1Vis + 4),
      vel1x > 0 ? const Color(0xFF6688FF) : const Color(0xFFFF4444),
      7,
    );
    _drawLabel(
      canvas,
      dopplerStr2,
      Offset(x2 - 20, y2 + r2Vis + 4),
      vel2x > 0 ? const Color(0xFF6688FF) : const Color(0xFFFF4444),
      7,
    );

    // ---- Light Curve (eclipse binary) ----
    canvas.drawRect(
      Rect.fromLTWH(0, lcTop, size.width, size.height - lcTop),
    ...
    _drawLabel(
      canvas,
      '광도 곡선 (식쌍성)',
      Offset(size.width / 2 - 30, lcTop + 2),
    ...
    // Axis line
    canvas.drawLine(
      Offset(lcPadL, lcTop + lcH * 0.15),
      Offset(lcPadL + lcW, lcTop + lcH * 0.15),
      Paint()
    ...
    canvas.drawLine(
      Offset(markerX, lcTop + 2),
      Offset(markerX, lcTop + lcH),
      Paint()
