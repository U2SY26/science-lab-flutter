...
class _PhotodiodeScreenPainter extends CustomPainter {
  ...
  Color _wavelengthToColor(double nm) {
    ...
    if (nm < 440) {
      return Color.fromARGB(255, (0 + (440 - nm) / 60 * 130).round(), 0, 255);
    }
    if (nm < 490) {
      return Color.fromARGB(255, 0, ((nm - 440) / 50 * 255).round(), 255);
    }
    if (nm < 510) {
      return Color.fromARGB(255, 0, 255, (255 - (nm - 490) / 20 * 255).round());
    }
    if (nm < 580) {
      return Color.fromARGB(255, ((nm - 510) / 70 * 255).round(), 255, 0);
    }
    if (nm < 645) {
      return Color.fromARGB(255, 255, (255 - (nm - 580) / 65 * 255).round(), 0);
    }
    return Color.fromARGB(
      255,
      255,
      (0 + (700 - nm) / 55 * 60).round().clamp(0, 60),
      0,
    );
  }
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawRect(
      ...
      Paint()
        ..color = const Color(0xFF5A8A9A).withValues(alpha: 0.6)
        ..strokeWidth = 1.5
    ...
    canvas.drawLine(
      ...
      Paint()
        ..color = photonColor.withValues(alpha: 0.5)
        ..strokeWidth = 1.5,
    );
    canvas.drawLine(
      Offset(jLeft - 18, jTop + jH * 0.5 - 4),
      Offset(jLeft - 12, jTop + jH * 0.5),
      Paint()
        ..color = photonColor.withValues(alpha: 0.5)
        ..strokeWidth = 1.5,
    ...
    _drawLabel(
      canvas,
      'hν',
      Offset(jLeft - 25, jTop + jH * 0.5 - 14),
      photonColor,
      9,
    );
    ...
    canvas.drawLine(
      Offset(jLeft, jTop - 22),
      Offset(jLeft + jW, jTop - 22),
      wirePaint,
    );
    canvas.drawLine(
      Offset(jLeft + jW, jTop - 22),
      Offset(jLeft + jW, jTop),
      wirePaint,
    );
    ...
    for (int i = 0; i < 4; i++) {
      ...
      canvas.drawCircle(
        Offset(dx, dy),
        2.5,
        Paint()..color = const Color(0xFF64FF8C).withValues(alpha: 0.85),
      );
    }
    _drawLabel(
      canvas,
      'I (광전류)',
      Offset(w * 0.5, jTop - 30),
    ...
    // Axes
    canvas.drawLine(
      Offset(cvLeft, cvBottom),
      Offset(cvLeft + cvW, cvBottom),
      Paint()
        ..color = const Color(0xFF5A8A9A)
        ..strokeWidth = 1,
    );
    canvas.drawLine(
      Offset(cvMidX, cvTop2),
      Offset(cvMidX, cvBottom),
      Paint()
        ..color = const Color(0xFF5A8A9A)
        ..strokeWidth = 1,
    );
    _drawLabel(
      canvas,
      'V',
      Offset(cvLeft + cvW + 6, cvBottom - 4),
      const Color(0xFF5A8A9A),
      8,
    );
    _drawLabel(
    ...
    for (double x2 = 0; x2 <= cvW * 0.55; x2 += 2) {
      ...
      lightPath.lineTo(
        cvLeft + x2 + cvW * 0.45,
        cvMidY - iNorm * cvH * 0.5 + photoShift,
      );
    }
    canvas.drawPath(
      lightPath,
      Paint()
        ..color = photonColor.withValues(alpha: 0.8)
        ..strokeWidth = 1.5
        ..style = PaintingStyle.stroke,
    );
    ...
    _drawLabel(
      canvas,
      '암전류',
      Offset(cvLeft + cvW * 0.2, cvMidY - 4),
      const Color(0xFF5A8A9A),
      8,
    );
    _drawLabel(
      canvas,
      '광전류',
      Offset(cvLeft + cvW * 0.2, cvMidY + photoShift + 4),
      photonColor,
      8,
    );
    ...
    _drawLabel(
      canvas,
      'I=${current.toStringAsFixed(0)}μA  λ=${wavelength.toStringAsFixed(0)}nm',
      Offset(w * 0.73, h * 0.92),
