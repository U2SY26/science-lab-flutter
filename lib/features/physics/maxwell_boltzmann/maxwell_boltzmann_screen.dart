...
class _MaxwellBoltzmannScreenState extends State<MaxwellBoltzmannScreen> {
  ...
  double get mostProbableSpeed =>
      math.sqrt(2 * 8.314 * temperature / (molarMass / 1000));
  double get meanSpeed =>
      math.sqrt(8 * 8.314 * temperature / (math.pi * molarMass / 1000));
  double get rmsSpeed =>
      math.sqrt(3 * 8.314 * temperature / (molarMass / 1000));
      ...
      appBar: AppBar(
        ...
        title: Column(
          ...
          children: [
            ...
            Text(
              isKorean ? '맥스웰-볼츠만 분포' : 'Maxwell-Boltzmann Distribution',
            ...
            children: [
              PresetGroup(
                ...
                presets: [
                  PresetButton(
                    label: 'H₂ (2)',
                    isSelected: molarMass == 2,
                    onPressed: () => setState(() => molarMass = 2),
                  ),
                  PresetButton(
                    label: 'N₂ (28)',
                    isSelected: molarMass == 28,
                    onPressed: () => setState(() => molarMass = 28),
                  ),
                  PresetButton(
                    label: 'O₂ (32)',
                    isSelected: molarMass == 32,
                    onPressed: () => setState(() => molarMass = 32),
                  ),
                  PresetButton(
                    label: 'CO₂ (44)',
                    isSelected: molarMass == 44,
                    onPressed: () => setState(() => molarMass = 44),
                  ),
                ],
              ),
                    ...
                    Expanded(
                      child: Column(
                        children: [
                          ...
                          Text(
                            '${mostProbableSpeed.toStringAsFixed(0)} m/s',
                            style: const TextStyle(
                              color: Colors.green,
                              fontSize: 11,
                    ...
                    Expanded(
                      child: Column(
                        children: [
                          ...
                          Text(
                            '${meanSpeed.toStringAsFixed(0)} m/s',
                            style: const TextStyle(
                              color: Colors.orange,
                              fontSize: 11,
                              fontFamily: 'monospace',
                    ...
                    Expanded(
                      child: Column(
                        children: [
                          ...
                          Text(
                            '${rmsSpeed.toStringAsFixed(0)} m/s',
                            style: TextStyle(
                              color: AppColors.accent,
                              fontSize: 11,
                              fontFamily: 'monospace',
...
class _MaxwellBoltzmannPainter extends CustomPainter {
  ...
  _MaxwellBoltzmannPainter({
    required this.temperature,
    required this.molarMass,
    required this.mostProbableSpeed,
    required this.meanSpeed,
    required this.rmsSpeed,
  ...
  void paint(Canvas canvas, Size size) {
    ...
    _drawText(
      canvas,
      'v (m/s)',
      Offset(graphX + graphWidth - 40, graphY + graphHeight + 10),
      AppColors.muted,
      10,
    );
    _drawText(
    ...
    for (int i = 0; i < 3; i++) {
      ...
      _drawText(
        canvas,
        labels[i],
        Offset(x - 10, graphY + graphHeight + 5),
        colors[i],
        10,
      );
    }
    ...
    _drawText(
      canvas,
      isKorean
          ? '온도: ${temperature.toStringAsFixed(0)}K, 분자량: ${molarMass.toStringAsFixed(0)} g/mol'
          : 'T: ${temperature.toStringAsFixed(0)}K, M: ${molarMass.toStringAsFixed(0)} g/mol',
      Offset(graphX + 10, graphY + 10),
      AppColors.ink,
      11,
    );
  }
