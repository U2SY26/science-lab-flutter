...
class _CoulombLawScreenState extends State<CoulombLawScreen> {
  ...
  double get force =>
      k * (charge1 * 1e-6) * (charge2 * 1e-6) / (distance * distance);
      ...
      body: SingleChildScrollView(
        ...
        child: SimulationContainer(
          ...
          controls: Column(
            ...
            children: [
              SimSlider(
                label: isKorean ? '전하 1 (q₁)' : 'Charge 1 (q₁)',
                value: charge1,
                min: -5,
                max: 5,
                defaultValue: 1,
                formatValue: (v) => '${v.toStringAsFixed(1)} μC',
                onChanged: (v) => setState(() => charge1 = v),
              ),
              const SizedBox(height: 12),
              SimSlider(
                label: isKorean ? '전하 2 (q₂)' : 'Charge 2 (q₂)',
                value: charge2,
                min: -5,
                max: 5,
                defaultValue: 1,
                formatValue: (v) => '${v.toStringAsFixed(1)} μC',
                onChanged: (v) => setState(() => charge2 = v),
              ),
              const SizedBox(height: 12),
              SimSlider(
                label: isKorean ? '거리 (r)' : 'Distance (r)',
                value: distance,
                min: 0.01,
                max: 0.5,
                step: 0.01,
                defaultValue: 0.1,
                formatValue: (v) => '${(v * 100).toStringAsFixed(0)} cm',
              ...
              Container(
                ...
                child: Column(
                  children: [
                        ...
                        Expanded(
                          child: Column(
                            children: [
                              ...
                              Text(
                                isRepulsive
                                    ? (isKorean ? '척력' : 'Repulsive')
                                    : (isKorean ? '인력' : 'Attractive'),
                                style: TextStyle(
                                  color: isRepulsive ? Colors.red : Colors.blue,
                                  fontSize: 12,
                                  fontWeight: FontWeight.w600,
                                ),
                              ),
                    ...
                    Text(
                      'F = 8.99×10⁹ × ${charge1.toStringAsFixed(1)}×10⁻⁶ × ${charge2.toStringAsFixed(1)}×10⁻⁶ / ${distance.toStringAsFixed(2)}²',
                      style: const TextStyle(
                        color: AppColors.accent,
                        fontSize: 10,
                        fontFamily: 'monospace',
...
class _CoulombLawPainter extends CustomPainter {
  ...
  _CoulombLawPainter({
    ...
    required this.charge2,
    required this.distance,
    required this.force,
    required this.isRepulsive,
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (isRepulsive) {
      _drawArrow(
        canvas,
        Offset(charge1X - 25, centerY),
        Offset(charge1X - 25 - arrowLength, centerY),
        Colors.red,
      );
      _drawArrow(
        canvas,
        Offset(charge2X + 25, centerY),
        Offset(charge2X + 25 + arrowLength, centerY),
        Colors.red,
      );
    } else {
      _drawArrow(
    ...
    // Distance line
    canvas.drawLine(
      Offset(charge1X, centerY + 50),
      Offset(charge2X, centerY + 50),
      Paint()
        ..color = AppColors.muted
        ..strokeWidth = 1,
    );
    _drawText(
      canvas,
      'r = ${(distance * 100).toStringAsFixed(0)} cm',
      Offset(size.width / 2 - 30, centerY + 55),
      AppColors.muted,
      10,
    );
    ...
    _drawText(
      canvas,
      'F = kq₁q₂/r² = ${force.toStringAsFixed(2)} N',
      Offset(20, 20),
      AppColors.ink,
      12,
    );
  ...
  void _drawCharge(Canvas canvas, Offset center, double charge, String label) {
    ...
    _drawText(
      canvas,
      label,
      Offset(center.dx - 8, center.dy + radius + 5),
      AppColors.muted,
      10,
    );
  }
