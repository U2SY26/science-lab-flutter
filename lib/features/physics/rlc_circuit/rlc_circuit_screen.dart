...
class _RlcCircuitScreenState extends State<RlcCircuitScreen>
  ...
  double get impedance =>
      math.sqrt(resistance * resistance + math.pow(xl - xc, 2));
  ...
  void initState() {
    ...
    _controller =
        AnimationController(
          ...
          ..addListener(() {
            if (isRunning) setState(() => time += 0.02);
          })
          ..repeat();
  }
      ...
      body: SingleChildScrollView(
        ...
        child: SimulationContainer(
          ...
          formulaDescription: isKorean
              ? 'RLC 회로의 임피던스와 공진 현상을 탐구합니다.'
              : 'Explore impedance and resonance in RLC circuits.',
          ...
          controls: Column(
            ...
            children: [
              SimSlider(
                label: isKorean ? '저항 (R)' : 'Resistance (R)',
                value: resistance,
                min: 10,
                max: 500,
                defaultValue: 100,
              ...
              SimSlider(
                ...
                onChanged: (v) => setState(() => inductance = v / 1000),
              ),
              const SizedBox(height: 8),
              SimSlider(
                ...
                onChanged: (v) => setState(() => capacitance = v * 1e-6),
              ),
              const SizedBox(height: 8),
              SimSlider(
                        ...
                        Expanded(
                          child: Column(
                            children: [
                              ...
                              Text(
                                qualityFactor.toStringAsFixed(2),
                                style: TextStyle(
                                  color: Colors.orange,
                                  fontSize: 11,
                                  fontFamily: 'monospace',
                        ...
                        Expanded(
                          child: Column(
                            children: [
                              ...
                              Text(
                                '${(phase * 180 / math.pi).toStringAsFixed(1)}°',
                                style: TextStyle(
                                  color: Colors.red,
                                  fontSize: 11,
                                  fontFamily: 'monospace',
              ...
              SimButtonGroup(
                ...
                buttons: [
                  ...
                  SimButton(
                    label: isKorean ? '공진' : 'Resonate',
                    icon: Icons.graphic_eq,
                    onPressed: () => setState(() => frequency = resonantFreq),
                  ),
                ],
              ),
            ],
...
class _RlcCircuitPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawLine(
      Offset(340, circuitY),
      Offset(340, circuitY + 40),
      Paint()
        ..color = AppColors.muted
        ..strokeWidth = 2,
    );
    canvas.drawLine(
      Offset(40, circuitY + 40),
      Offset(340, circuitY + 40),
      Paint()
        ..color = AppColors.muted
        ..strokeWidth = 2,
    ...
    canvas.drawLine(
      Offset(35, circuitY + 20),
      Offset(40, circuitY + 15),
      Paint()
        ..color = Colors.red
        ..strokeWidth = 2,
    );
    ...
    for (double x = 0; x <= graphWidth; x += 2) {
      ...
      if (x == 0) {
        voltagePath.moveTo(30 + x, y);
      } else {
        voltagePath.lineTo(30 + x, y);
      }
    }
    canvas.drawPath(
      voltagePath,
    ...
    _drawText(
      canvas,
      'V',
      Offset(35, graphY - graphHeight - 15),
      Colors.blue,
      11,
    );
    _drawText(
      canvas,
      'I',
      Offset(55, graphY - graphHeight - 15),
      Colors.red,
      11,
    );
    ...
    canvas.drawLine(
      Offset(phasorX, phasorY),
      Offset(
        phasorX + phasorRadius * math.cos(vAngle),
        phasorY - phasorRadius * math.sin(vAngle),
      ),
      Paint()
        ..color = Colors.blue
        ..strokeWidth = 3,
    ...
    canvas.drawLine(
      Offset(phasorX, phasorY),
      Offset(
        phasorX + phasorRadius * 0.8 * math.cos(iAngle),
        phasorY - phasorRadius * 0.8 * math.sin(iAngle),
      ),
      Paint()
        ..color = Colors.red
        ..strokeWidth = 3,
    ...
    _drawText(
      canvas,
      isKorean ? '위상도' : 'Phasor',
      Offset(phasorX - 20, phasorY + phasorRadius + 5),
      AppColors.muted,
      10,
    );
  }
  ...
  void _drawInductor(Canvas canvas, Offset center, String label) {
    for (int i = 0; i < 4; i++) {
      canvas.drawArc(
        Rect.fromCenter(
          center: Offset(center.dx - 15 + i * 10, center.dy),
          width: 10,
          height: 15,
        ),
        math.pi,
