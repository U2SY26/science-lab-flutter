...
class _SurfaceTensionScreenPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Water body fill
    canvas.drawRect(
      Rect.fromLTWH(0, waterTop + 10, w, waterBottom - waterTop),
    ...
    final cx = w * 0.5;
    final rippleAmp =
        6.0 * gamma / 0.072; // more surface tension = bigger ripple
    for (int i = 0; i < 4; i++) {
    ...
    for (int px = 0; px <= w.toInt(); px++) {
      ...
      if (px == 0) {
        surfacePath.moveTo(0, sy);
      } else {
        surfacePath.lineTo(px.toDouble(), sy);
      }
    }
    canvas.drawPath(
      surfacePath,
    ...
    canvas.drawLine(
      Offset(leftAx, arrowY),
      Offset(leftAx - arrowLen, arrowY + 4),
      Paint()
    ...
    // Right arrow
    canvas.drawLine(
      Offset(rightAx, arrowY),
      Offset(rightAx + arrowLen, arrowY + 4),
      Paint()
        ..color = const Color(0xFFFF6B35)
        ..strokeWidth = 1.5
        ..strokeCap = StrokeCap.round,
    ...
    _label(
      canvas,
      '모세관 현상 (γ=${gamma.toStringAsFixed(3)} N/m)',
      Offset(6, capY + 3),
      const Color(0xFF5A8A9A),
      8,
      bold: true,
    ...
    for (int ti = 0; ti < 3; ti++) {
      ...
      final riseH = (2 * gamma * 0.95 / (rhoG * r * 0.001)).clamp(
        0.0,
        capH * 0.7,
      );
      ...
      // Tube outline
      canvas.drawRect(
        Rect.fromLTWH(tx - r, capY + 14, r * 2, capH - 22),
        Paint()
          ..color = const Color(0xFF1A3040)
          ..strokeWidth = 1
          ..style = PaintingStyle.stroke,
      ...
      if (riseH > 1) {
        canvas.drawRect(
          Rect.fromLTWH(tx - r + 1, waterTopCapY, r * 2 - 2, riseH),
  ...
  @override
  bool shouldRepaint(covariant _SurfaceTensionScreenPainter oldDelegate) =>
      true;
}
