...
class _MotorGeneratorScreenState extends State<MotorGeneratorScreen>
  ...
  double get torque =>
      isMotorMode ? magneticField * current * 0.1 : 0; // Simplified
  double get emf =>
      isMotorMode ? 0 : magneticField * (rpm / 60) * 0.1; // Simplified
  double get angularVelocity =>
      isMotorMode ? torque * 100 : rpm * 2 * math.pi / 60;
      ...
      appBar: AppBar(
        ...
        title: Column(
          ...
          children: [
            ...
            Text(
              isKorean ? '모터와 발전기' : 'Motor & Generator',
      ...
      body: SingleChildScrollView(
        ...
        child: SimulationContainer(
          ...
          formulaDescription: isKorean
              ? (isMotorMode ? '모터: 전류가 자기장에서 힘을 받아 회전' : '발전기: 코일 회전으로 전자기 유도')
              : (isMotorMode
                    ? 'Motor: Current in magnetic field creates rotation'
                    : 'Generator: Rotating coil induces EMF'),
          simulation: CustomPaint(
            painter: _MotorGeneratorPainter(
          ...
          controls: Column(
            ...
            children: [
              SimSegment<bool>(
                ...
                options: {
                  true: isKorean ? '모터' : 'Motor',
                  false: isKorean ? '발전기' : 'Generator',
                },
                selected: isMotorMode,
                onChanged: (v) => setState(() => isMotorMode = v),
              ),
              const SizedBox(height: 16),
              SimSlider(
                label: isKorean ? '자기장 (B)' : 'Magnetic Field (B)',
                value: magneticField,
                min: 0.1,
                max: 1.0,
                defaultValue: 0.5,
              ...
              if (isMotorMode)
                ...
                SimSlider(
                  label: isKorean ? '회전속도' : 'RPM',
                  value: rpm,
                  min: 100,
                  max: 3000,
                  defaultValue: 1000,
                  formatValue: (v) => '${v.toStringAsFixed(0)} RPM',
                  onChanged: (v) => setState(() => rpm = v),
                ),
              const SizedBox(height: 16),
              Container(
                          ...
                          Expanded(
                            child: Column(
                              children: [
                                ...
                                Text(
                                  '${(torque * 1000).toStringAsFixed(1)} mN·m',
                                  style: TextStyle(
                                    color: AppColors.accent,
                                    fontSize: 11,
                          ...
                          Expanded(
                            child: Column(
                              children: [
                                ...
                                Text(
                                  '${(angularVelocity * 60 / (2 * math.pi)).toStringAsFixed(0)} RPM',
                                  style: TextStyle(
                          ...
                          Expanded(
                            child: Column(
                              children: [
                                ...
                                Text(
                                  '${(emf * math.sin(rotorAngle)).toStringAsFixed(2)} V',
                                  style: TextStyle(
                                    color: AppColors.accent,
                                    fontSize: 11,
                                    fontFamily: 'monospace',
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawRRect(
      RRect.fromRectAndRadius(
        Rect.fromLTWH(centerX - radius - 40, centerY - radius, 30, radius * 2),
    ...
    canvas.drawRRect(
      RRect.fromRectAndRadius(
        Rect.fromLTWH(centerX + radius + 10, centerY - radius, 30, radius * 2),
        const Radius.circular(5),
      ),
    ...
    _drawText(
      canvas,
      'N',
      Offset(centerX - radius - 32, centerY - 8),
      Colors.white,
      16,
    );
    _drawText(
      canvas,
      'S',
      Offset(centerX + radius + 18, centerY - 8),
      Colors.white,
      16,
    );
    ...
    for (int i = -2; i <= 2; i++) {
      canvas.drawLine(
        Offset(centerX - radius - 10, centerY + i * 20),
        Offset(centerX + radius + 10, centerY + i * 20),
        Paint()
    ...
    canvas.drawRect(
      Rect.fromLTWH(centerX - 5, centerY + radius + 10, 10, 20),
    ...
    if (!isMotorMode) {
      ...
      for (double x = 0; x <= graphWidth; x += 2) {
        ...
        if (x == 0) {
          emfPath.moveTo(30 + x, y);
        } else {
          emfPath.lineTo(30 + x, y);
        }
      }
      canvas.drawPath(
        emfPath,
      ...
      _drawText(
        canvas,
        '${emfMax.toStringAsFixed(2)} V (max)',
        Offset(size.width - 100, graphY - 50),
        AppColors.muted,
        10,
      );
    }
    ...
    if (isMotorMode) {
      ...
      canvas.drawLine(
        Offset(coilX1, coilY1),
        Offset(
          coilX1 + forceScale * math.cos(forceDir),
          coilY1 + forceScale * math.sin(forceDir),
        ),
        Paint()
          ..color = Colors.green
          ..strokeWidth = 3,
