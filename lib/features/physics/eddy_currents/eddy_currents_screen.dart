  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawRect(
      ...
      Paint()
        ..color = const Color(
          0xFF00D4FF,
        ).withValues(alpha: 0.05 + bField * 0.04),
    );
    ...
    for (int r = 0; r < bRows; r++) {
      for (int c = 0; c < bCols; c++) {
        ...
        canvas.drawCircle(
          Offset(bx, by),
          0.8,
          Paint()..color = const Color(0xFF00D4FF).withValues(alpha: 0.5),
        );
      }
    }
    ...
    if (rawY > bTop - 20 && rawY < bBot + 20) {
      ...
      solidY =
          bTop -
          20 +
          enterProgress * (bBot - bTop + 40) * slowFactor.clamp(0.1, 1.0) +
    ...
    if (inBField) {
      ...
      final eddyPaint = Paint()
        ..color = const Color(0xFFFF6B35).withValues(alpha: eddyAlpha)
        ..strokeWidth = 1.5
        ..style = PaintingStyle.stroke;
      ...
      for (final r2 in [diskR * 0.35, diskR * 0.65]) {
        ...
        canvas.drawCircle(
          Offset(ax, ay),
          2,
          Paint()..color = const Color(0xFFFF6B35).withValues(alpha: eddyAlpha),
        );
      }
      // Braking force arrow (upward)
      canvas.drawLine(
        Offset(sdCx, sdCy + diskR + 2),
        Offset(sdCx, sdCy + diskR + 14),
        Paint()
          ..color = const Color(0xFF64FF8C)
          ..strokeWidth = 2,
      );
      canvas.drawLine(
        Offset(sdCx - 4, sdCy + diskR + 8),
        Offset(sdCx, sdCy + diskR + 2),
        Paint()
          ..color = const Color(0xFF64FF8C)
          ..strokeWidth = 2,
    ...
    for (int s = 0; s < 6; s++) {
      ...
      canvas.drawLine(
        Offset(
          slCx + diskR * 0.3 * math.cos(sAngle),
          slCy + diskR * 0.3 * math.sin(sAngle),
        ),
        Offset(
          slCx + diskR * 0.9 * math.cos(sAngle),
          slCy + diskR * 0.9 * math.sin(sAngle),
        ),
        Paint()
    ...
    _drawLabel(
      canvas,
      'B=${bField.toStringAsFixed(2)}T  f=${freq.toStringAsFixed(0)}Hz',
      Offset(w / 2, h * 0.93),
      const Color(0xFF5A8A9A),
      9,
    );
