...
class _EmWaveScreenState extends State<EmWaveScreen>
  ...
  void initState() {
    ...
    _controller =
        AnimationController(
          ...
          ..addListener(() {
            if (isRunning) setState(() => time += 0.05);
          })
          ..repeat();
  }
      ...
      body: SingleChildScrollView(
        ...
        child: SimulationContainer(
          ...
          formulaDescription: isKorean
              ? '전기장(E)과 자기장(B)이 수직으로 진동하며 전파됩니다.'
              : 'Electric (E) and magnetic (B) fields oscillate perpendicular to propagation.',
                ...
                children: [
                  Expanded(
                    child: CheckboxListTile(
                      title: Text(
                        isKorean ? '전기장 (E)' : 'E Field',
                        style: TextStyle(color: Colors.red, fontSize: 12),
                      ),
                      value: showE,
                      onChanged: (v) => setState(() => showE = v!),
                      dense: true,
                  ...
                  Expanded(
                    child: CheckboxListTile(
                      title: Text(
                        isKorean ? '자기장 (B)' : 'B Field',
                        style: TextStyle(color: Colors.blue, fontSize: 12),
                      ),
                      value: showB,
                      onChanged: (v) => setState(() => showB = v!),
                      dense: true,
                      contentPadding: EdgeInsets.zero,
                    ),
                  ),
                        ...
                        Expanded(
                          child: Column(
                            children: [
                              ...
                              Text(
                                '${(frequency / 1e12).toStringAsFixed(1)} THz',
                                style: TextStyle(
                                  color: AppColors.accent,
                                  fontSize: 11,
                        ...
                        Expanded(
                          child: Column(
                            children: [
                              ...
                              Text(
                                '${(energy / 1.6e-19).toStringAsFixed(2)} eV',
                                style: TextStyle(
...
class _EmWavePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    _drawText(
      canvas,
      'z',
      Offset(size.width - 25, centerY + 10),
      AppColors.muted,
      12,
    );
    ...
    if (showE) {
      ...
      for (double x = 0; x <= waveWidth; x += 2) {
        ...
        if (x == 0) {
          ePath.moveTo(screenX, screenY);
        } else {
          ePath.lineTo(screenX, screenY);
        }
      }
      canvas.drawPath(
        ePath,
      ...
      _drawText(
        canvas,
        'E',
        Offset(25, centerY - waveAmplitude - 15),
        Colors.red,
        14,
      );
    }
    ...
    if (showB) {
      ...
      for (double x = 0; x <= waveWidth; x += 2) {
        ...
        if (x == 0) {
          bPath.moveTo(screenX, screenY);
        } else {
          bPath.lineTo(screenX, screenY);
        }
      }

      // Draw B field axis
      canvas.drawLine(
        Offset(20, centerY + 80),
        Offset(size.width - 20, centerY + 80),
        Paint()
      ...
      _drawText(
        canvas,
        'B',
        Offset(25, centerY + 80 - waveAmplitude * 0.6 - 15),
        Colors.blue,
        14,
      );
    }
    ...
    for (double x = 0; x < spectrumWidth; x++) {
      ...
      canvas.drawLine(
        Offset(30 + x, spectrumY - 10),
        Offset(30 + x, spectrumY + 10),
        Paint()..color = _getColorForWavelength(wl),
      );
    }
    ...
    _drawText(
      canvas,
      '750nm',
      Offset(size.width - 55, spectrumY + 15),
      AppColors.muted,
      9,
    );
    _drawText(
      canvas,
      isKorean ? '가시광선 스펙트럼' : 'Visible Spectrum',
      Offset(size.width / 2 - 50, spectrumY + 15),
      AppColors.muted,
      10,
    );
  }
  ...
  Color _getColorForWavelength(double wl) {
    ...
    if (wl < 450) {
      return Color.lerp(Colors.purple, Colors.blue, (wl - 380) / 70)!;
    }
    if (wl < 495) return Color.lerp(Colors.blue, Colors.cyan, (wl - 450) / 45)!;
    if (wl < 570) {
      return Color.lerp(Colors.cyan, Colors.green, (wl - 495) / 75)!;
    }
    if (wl < 590) {
      return Color.lerp(Colors.green, Colors.yellow, (wl - 570) / 20)!;
    }
    if (wl < 620) {
      return Color.lerp(Colors.yellow, Colors.orange, (wl - 590) / 30)!;
    }
    if (wl < 750) {
      return Color.lerp(Colors.orange, Colors.red, (wl - 620) / 130)!;
    }
    return Colors.red.shade900;
  }
  ...
  void _drawArrow(Canvas canvas, Offset from, Offset to, Color color) {
    ...
    canvas.drawLine(
      to,
      Offset(
        to.dx - 8 * math.cos(angle - 0.5),
        to.dy - 8 * math.sin(angle - 0.5),
      ),
      Paint()
        ..color = color
        ..strokeWidth = 2,
