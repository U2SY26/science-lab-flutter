...
class _TransformerScreenState extends State<TransformerScreen>
  ...
  double get secondaryCurrent =>
      primaryCurrent / turnsRatio; // Power conservation
  ...
  void initState() {
    ...
    _controller =
        AnimationController(
          ...
          ..addListener(() {
            if (isRunning) setState(() => time += 0.03);
          })
          ..repeat();
  }
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Column(
          ...
          children: [
            Text(
              isKorean ? '전자기학' : 'ELECTROMAGNETISM',
              style: TextStyle(
      ...
      body: SingleChildScrollView(
        ...
        child: SimulationContainer(
          ...
          formulaDescription: isKorean
              ? '1차 코일과 2차 코일의 권선비에 따라 전압이 변환됩니다.'
              : 'Voltage transforms according to the turns ratio of primary and secondary coils.',
          ...
          controls: Column(
            ...
            children: [
              SimSlider(
                label: isKorean ? '1차 전압 (Vp)' : 'Primary Voltage (Vp)',
                value: primaryVoltage,
                min: 10,
                max: 240,
                defaultValue: 120,
                formatValue: (v) => '${v.toStringAsFixed(0)} V',
                onChanged: (v) => setState(() => primaryVoltage = v),
              ),
              const SizedBox(height: 12),
              SimSlider(
                label: isKorean ? '1차 권선수 (Np)' : 'Primary Turns (Np)',
                value: primaryTurns.toDouble(),
                min: 10,
                max: 200,
                defaultValue: 100,
                formatValue: (v) => v.toStringAsFixed(0),
                onChanged: (v) => setState(() => primaryTurns = v.round()),
              ),
              const SizedBox(height: 12),
              SimSlider(
                label: isKorean ? '2차 권선수 (Ns)' : 'Secondary Turns (Ns)',
                value: secondaryTurns.toDouble(),
                min: 10,
                max: 200,
                defaultValue: 50,
                formatValue: (v) => v.toStringAsFixed(0),
                onChanged: (v) => setState(() => secondaryTurns = v.round()),
              ),
                        ...
                        Expanded(
                          child: Column(
                            children: [
                              ...
                              Text(
                                '${secondaryVoltage.toStringAsFixed(1)} V',
                                style: TextStyle(
                                  color: AppColors.accent,
                                  fontSize: 11,
                        ...
                        Expanded(
                          child: Column(
                            children: [
                              ...
                              Text(
                                '${secondaryCurrent.toStringAsFixed(2)} A',
                                style: TextStyle(
                                  color: Colors.purple,
                                  fontSize: 11,
                                  fontFamily: 'monospace',
...
class _TransformerPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawRect(
      Rect.fromLTWH(centerX - 70, centerY - 60, 140, 20),
      Paint()..color = Colors.grey[600]!,
    );
    canvas.drawRect(
      Rect.fromLTWH(centerX - 70, centerY + 40, 140, 20),
      Paint()..color = Colors.grey[600]!,
    );
    ...
    for (int i = 0; i < math.min(primaryTurns ~/ 10, 10); i++) {
      ...
      canvas.drawArc(
        Rect.fromCenter(
          center: Offset(primaryCoilX - 15, y),
          width: 30,
          height: 12,
        ),
        -math.pi / 2,
    ...
    _drawText(
      canvas,
      isKorean ? '1차' : 'Primary',
      Offset(primaryCoilX - 35, centerY + 70),
      Colors.red,
      10,
    );
    _drawText(
      canvas,
      'Np=$primaryTurns',
      Offset(primaryCoilX - 35, centerY + 82),
      Colors.red,
      9,
    );
    ...
    for (int i = 0; i < math.min(secondaryTurns ~/ 10, 10); i++) {
      ...
      canvas.drawArc(
        Rect.fromCenter(
          center: Offset(secondaryCoilX + 15, y),
          width: 30,
          height: 12,
        ),
        math.pi / 2,
    ...
    _drawText(
      canvas,
      isKorean ? '2차' : 'Secondary',
      Offset(secondaryCoilX, centerY + 70),
      Colors.blue,
      10,
    );
    _drawText(
      canvas,
      'Ns=$secondaryTurns',
      Offset(secondaryCoilX, centerY + 82),
      Colors.blue,
      9,
    );
    ...
    for (int i = 0; i < 3; i++) {
      ...
      canvas.drawLine(
        Offset(centerX - 60, centerY - 40 + i * 40),
        Offset(centerX + 60, centerY - 40 + i * 40),
        Paint()
    ...
    // Input AC symbol
    canvas.drawCircle(
      Offset(primaryCoilX - 50, centerY),
      15,
      Paint()
        ..color = Colors.red
        ..style = PaintingStyle.stroke
    ...
    final acPath = Path()
      ..moveTo(primaryCoilX - 58, centerY)
      ..quadraticBezierTo(
        primaryCoilX - 50,
        centerY - 8,
        primaryCoilX - 50,
    ...
    _drawText(
      canvas,
      '${primaryVoltage.toStringAsFixed(0)}V',
      Offset(primaryCoilX - 65, centerY + 20),
      Colors.red,
      10,
    );
    ...
    // Output indicator
    canvas.drawCircle(
      Offset(secondaryCoilX + 50, centerY),
      15,
      Paint()
        ..color = Colors.blue
        ..style = PaintingStyle.stroke
    ...
    canvas.drawCircle(
      ...
      Paint()
        ..color = Colors.yellow.withValues(
          alpha: brightness * secondaryVoltage / 240,
        ),
    );
    _drawText(
      canvas,
      '${secondaryVoltage.toStringAsFixed(0)}V',
      Offset(secondaryCoilX + 35, centerY + 20),
      Colors.blue,
      10,
    );
    ...
    for (double x = 0; x <= graphWidth / 2 - 10; x += 2) {
      ...
      if (x == 0) {
        primaryPath.moveTo(30 + x, y);
      } else {
        primaryPath.lineTo(30 + x, y);
      }
    }
    canvas.drawPath(
      primaryPath,
    ...
    for (double x = 0; x <= graphWidth / 2 - 10; x += 2) {
      ...
      if (x == 0) {
        secondaryPath.moveTo(graphWidth / 2 + 30 + x, y);
      } else {
        secondaryPath.lineTo(graphWidth / 2 + 30 + x, y);
      }
    }
    canvas.drawPath(
      secondaryPath,
    ...
    _drawText(
      canvas,
      'Vs',
      Offset(graphWidth / 2 + 35, graphY - 45),
      Colors.blue,
      10,
    );
  }
