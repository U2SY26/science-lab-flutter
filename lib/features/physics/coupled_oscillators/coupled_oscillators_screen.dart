...
class CoupledOscillatorsScreen extends StatefulWidget {
  ...
  @override
  State<CoupledOscillatorsScreen> createState() =>
      _CoupledOscillatorsScreenState();
}
              ...
              Container(
                ...
                child: Row(
                  children: [
                    ...
                    _V(
                      'ω+',
                      math.sqrt((_k1 + 2 * _coupling)).toStringAsFixed(2),
                    ),
                  ],
                ),
              ),
...
class _CoupledOscillatorsScreenPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int s = 0; s < 5; s++) {
      canvas.drawLine(
        Offset(wallL, springBase - 12 + s * 5),
        Offset(wallL + 6, springBase - 12 + s * 5 - 4),
        Paint()
          ..color = const Color(0xFF5A8A9A).withValues(alpha: 0.5)
          ..strokeWidth = 1,
      );
      canvas.drawLine(
        Offset(wallR, springBase - 12 + s * 5),
        Offset(wallR - 6, springBase - 12 + s * 5 - 4),
        Paint()
          ..color = const Color(0xFF5A8A9A).withValues(alpha: 0.5)
          ..strokeWidth = 1,
      );
    ...
    canvas.drawLine(
      Offset(wallL, springBase - 22),
      Offset(wallL, springBase + 6),
      Paint()
        ..color = const Color(0xFF5A8A9A)
        ..strokeWidth = 2,
    );
    canvas.drawLine(
      Offset(wallR, springBase - 22),
      Offset(wallR, springBase + 6),
      Paint()
        ..color = const Color(0xFF5A8A9A)
        ..strokeWidth = 2,
    );
    ...
    _text(
      canvas,
      'm₁',
      Offset(m1x - 6, springBase - 6),
      const TextStyle(
        color: Color(0xFF0D1A20),
        fontSize: 8,
    ...
    _text(
      canvas,
      '결합 진동자  k₁=${k1.toStringAsFixed(1)}  k₂=${coupling.toStringAsFixed(1)}',
      Offset(w / 2 - 72, 4),
      const TextStyle(
    ...
    canvas.drawRRect(
      RRect.fromRectAndRadius(
        Rect.fromLTWH(padL, specY + 10, bar1W, barH2),
    ...
    _text(
      canvas,
      'ω⁻=${omMinus.toStringAsFixed(2)}',
      Offset(padL + bar1W + 4, specY + 10),
      const TextStyle(color: Color(0xFF00D4FF), fontSize: 8),
    );

    canvas.drawRRect(
      RRect.fromRectAndRadius(
        Rect.fromLTWH(padL, specY + 10 + barH2 + 4, bar2W, barH2),
        const Radius.circular(2),
      ),
    ...
    _text(
      canvas,
      'ω⁺=${omPlus.toStringAsFixed(2)}',
      Offset(padL + bar2W + 4, specY + 10 + barH2 + 4),
      const TextStyle(color: Color(0xFFFF6B35), fontSize: 8),
    );
  }
  ...
  @override
  bool shouldRepaint(covariant _CoupledOscillatorsScreenPainter oldDelegate) =>
      true;
}
