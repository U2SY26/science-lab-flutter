...
class RelativisticEnergyScreen extends StatefulWidget {
  ...
  @override
  State<RelativisticEnergyScreen> createState() =>
      _RelativisticEnergyScreenState();
}
...
class _RelativisticEnergyScreenPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i <= 200; i++) {
      ...
      if (ey < pad - 5) break;
      if (classFirst) {
        classPath.moveTo(ex, ey);
        classFirst = false;
      } else {
        classPath.lineTo(ex, ey);
      }
    }
    ...
    canvas.drawPath(
      classPath,
      dashedPaint..color = const Color(0xFFFF6B35).withValues(alpha: 0.6),
    );
    ...
    for (int i = 0; i <= 400; i++) {
      ...
      if (ey < pad - 5) break;
      if (relFirst) {
        relPath.moveTo(ex, ey);
        relFirst = false;
      } else {
        relPath.lineTo(ex, ey);
      }
    }
    ...
    // γ label
    drawText(
      'γ=${gammaCur.toStringAsFixed(2)}',
      Offset(curX + 4, curY.clamp(pad, originY - 20).toDouble() - 14),
      color: const Color(0xFF00D4FF),
      fs: 9,
    );
    ...
    // Legend
    canvas.drawLine(
      Offset(originX + 8, pad + 8),
      Offset(originX + 24, pad + 8),
      Paint()
        ..color = const Color(0xFF00D4FF)
        ..strokeWidth = 2,
    );
    drawText(
      '상대론적 E=γmc²',
      Offset(originX + 26, pad + 3),
      color: const Color(0xFF00D4FF),
      fs: 8,
    );
    canvas.drawLine(
      Offset(originX + 8, pad + 20),
      Offset(originX + 24, pad + 20),
      Paint()
    ...
    drawText(
      '고전적 E₀+½mv²',
      Offset(originX + 26, pad + 15),
      color: const Color(0xFFFF6B35),
      fs: 8,
    );
    canvas.drawCircle(
      Offset(originX + 16, pad + 32),
      3,
      Paint()..color = const Color(0xFF64FF8C),
    );
    drawText(
      '운동량 p=γmv',
      Offset(originX + 26, pad + 27),
  ...
  @override
  bool shouldRepaint(covariant _RelativisticEnergyScreenPainter oldDelegate) =>
      true;
}
