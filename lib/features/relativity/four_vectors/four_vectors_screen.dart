...
class _FourVectorsScreenPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (double px2 = 0; px2 <= maxVal; px2 += 0.1) {
      ...
      if (hy < dT) break;
      if (hyperFirst) {
        hyperPath.moveTo(hx, hy);
        hyperFirst = false;
      } else {
        hyperPath.lineTo(hx, hy);
      }
    }
    canvas.drawPath(
      hyperPath,
    ...
    if (tipX <= dR && tipY >= dT) {
      ...
      _lbl(
        canvas,
        'px=${p.toStringAsFixed(2)}',
        Offset((ox + tipX) / 2, oy + 8),
    ...
    canvas.drawLine(
      Offset(ox, oy),
      Offset(ox + math.min(dW - 20, dH - 20), oy - math.min(dW - 20, dH - 20)),
      Paint()
    ...
    for (int ci = 0; ci < compData.length; ci++) {
      ...
      canvas.drawRRect(
        RRect.fromRectAndRadius(
          Rect.fromLTWH(bx, barTop2, barW6, barH4),
      ...
      _lbl(
        canvas,
        compData[ci].$1,
        Offset(bx + barW6 / 2, barBotY3 + 7),
        color2,
        8,
      );
      _lbl(
        canvas,
        compData[ci].$2.toStringAsFixed(2),
        Offset(bx + barW6 / 2, barBotY3 - fillH3 - 7),
        color2,
        7,
      );
    }
    ...
    _lbl(
      canvas,
      's² = E²-p² = m₀²',
      Offset(iL + iW / 2, invY + 13),
      const Color(0xFF5A8A9A),
      8,
    );
    _lbl(
      canvas,
      '= ${invMass.toStringAsFixed(3)}',
      Offset(iL + iW / 2, invY + 25),
      const Color(0xFF64FF8C),
      10,
      fw: FontWeight.bold,
    );
    ...
    _lbl(
      canvas,
      'v=${v.toStringAsFixed(2)}c  γ=${gamma.toStringAsFixed(3)}',
      Offset(iL + iW / 2, ltY),
      const Color(0xFF00D4FF),
      9,
    );
    _lbl(
      canvas,
      'E=γm₀=${E.toStringAsFixed(3)}',
      Offset(iL + iW / 2, ltY + 13),
      const Color(0xFFFF6B35),
      9,
    );
    _lbl(
      canvas,
      'p=γm₀v=${p.toStringAsFixed(3)}',
      Offset(iL + iW / 2, ltY + 26),
      const Color(0xFF64FF8C),
      9,
    );
