              ...
              Container(
                ...
                child: Row(
                  children: [
                    ...
                    _V(
                      '차이',
                      '${(_classical - _relativistic).toStringAsFixed(3)} c',
                    ),
                  ],
                ),
              ),
...
class _VelocityAdditionScreenPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int gi = 0; gi <= 4; gi++) {
      _lbl(
        canvas,
        (gi * 0.25).toStringAsFixed(2),
        Offset(gL + gi * (gW / 4), gB + 7),
        const Color(0xFF5A8A9A),
        7,
      );
      _lbl(
        canvas,
        (gi * 0.5).toStringAsFixed(1),
        Offset(gL - 14, gB - gi * (gH / 4)),
        const Color(0xFF5A8A9A),
        7,
      );
    }
    ...
    while (dashX < gR) {
      canvas.drawLine(
        Offset(dashX, cLineY),
        Offset(math.min(dashX + 5, gR), cLineY),
        Paint()
    ...
    for (double v1 = 0; v1 <= 0.99; v1 += 0.01) {
      ...
      if (v1 == 0) {
        classPath.moveTo(px, py);
      } else {
        classPath.lineTo(px, py);
      }
    }
    canvas.drawPath(
      classPath,
    ...
    for (double v1 = 0; v1 <= 0.99; v1 += 0.01) {
      ...
      if (v1 == 0) {
        relPath.moveTo(px, py);
      } else {
        relPath.lineTo(px, py);
      }
    }
    canvas.drawPath(
      relPath,
    ...
    canvas.drawCircle(
      Offset(curX, curClassY.clamp(gT, gB)),
      5,
      Paint()..color = const Color(0xFFFF6B35),
    );
    canvas.drawCircle(
      Offset(curX, curRelY.clamp(gT, gB)),
      5,
    ...
    canvas.drawLine(
      ...
      Offset(curX, gB),
      Paint()
        ..color = const Color(0xFFE0F4FF).withValues(alpha: 0.3)
        ..strokeWidth = 1
        ..style = PaintingStyle.stroke,
    );
    ...
    _lbl(
      canvas,
      'v₁=${u2.toStringAsFixed(2)}c  v₂=${v2.toStringAsFixed(2)}c',
      Offset(w / 2, barSect + 8),
    ...
    canvas.drawRRect(
      RRect.fromRectAndRadius(
        Rect.fromLTWH(barL2, classBarY, barW7, barH5),
    ...
    canvas.drawRRect(
      RRect.fromRectAndRadius(
        Rect.fromLTWH(barL2, classBarY, classBarW, barH5),
        const Radius.circular(3),
      ),
    ...
    canvas.drawLine(
      Offset(cMarkX, classBarY - 2),
      Offset(cMarkX, classBarY + barH5 + 2),
      Paint()
    ...
    _lbl(
      canvas,
      '고전 w=${classical.toStringAsFixed(3)}c',
      Offset(barL2 + barW7 / 2, classBarY + barH5 / 2),
      const Color(0xFFE0F4FF),
      9,
    );
    if (classical > 1.0) {
      _lbl(
        canvas,
        '광속 초과!',
        Offset(classBarW + barL2 + 16, classBarY + barH5 / 2),
    ...
    canvas.drawRRect(
      RRect.fromRectAndRadius(
        Rect.fromLTWH(barL2, relBarY, barW7, barH5),
        const Radius.circular(3),
      ),
      Paint()..color = const Color(0xFF1A3040),
    );
    canvas.drawRRect(
      RRect.fromRectAndRadius(
        Rect.fromLTWH(barL2, relBarY, relBarW, barH5),
        const Radius.circular(3),
      ),
      Paint()..color = const Color(0xFF00D4FF).withValues(alpha: 0.8),
    );
    canvas.drawLine(
      Offset(cMarkX, relBarY - 2),
      Offset(cMarkX, relBarY + barH5 + 2),
      Paint()
        ..color = const Color(0xFFFFD700)
        ..strokeWidth = 1.5,
    );
    _lbl(
      canvas,
      '상대론 w=${relativistic.toStringAsFixed(3)}c',
      Offset(barL2 + barW7 / 2, relBarY + barH5 / 2),
      const Color(0xFFE0F4FF),
      9,
    );
    ...
    _lbl(
      canvas,
      '차이: ${diff.toStringAsFixed(3)}c  (${(diff / classical * 100).toStringAsFixed(1)}%)',
      Offset(w / 2, diffY),
  ...
  @override
  bool shouldRepaint(covariant _VelocityAdditionScreenPainter oldDelegate) =>
      true;
}
