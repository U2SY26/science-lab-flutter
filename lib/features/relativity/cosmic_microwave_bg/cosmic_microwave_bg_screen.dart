...
class CosmicMicrowaveBgScreen extends StatefulWidget {
  ...
  @override
  State<CosmicMicrowaveBgScreen> createState() =>
      _CosmicMicrowaveBgScreenState();
}
              ...
              Container(
                ...
                child: Row(
                  children: [
                    ...
                    _V(
                      'ΔT/T',
                      '${(_anisotropy * 1e5).toStringAsFixed(1)}×10⁻⁵',
                    ),
...
class _CosmicMicrowaveBgScreenPainter extends CustomPainter {
  ...
  _CosmicMicrowaveBgScreenPainter({required this.time, required this.tempCmb});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int row = 0; row < rows; row++) {
      for (int col = 0; col < cols; col++) {
        ...
        deltaT =
            (math.sin(lon * 2 + lat * 1.5 + 0.3) * 0.5 +
    ...
    _drawLabel(
      canvas,
      'T = ${tempCmb.toStringAsFixed(3)} K',
      Offset(mapLeft + mapW / 2, mapTop + mapH + 10),
      11,
      AppColors.muted,
    );
    ...
    canvas.drawLine(
      ...
      Offset(graphLeft + graphW, graphBottom),
      axisPaint,
    );
    canvas.drawLine(
    ...
    _drawLabel(
      canvas,
      'ℓ',
      Offset(graphLeft + graphW - 6, graphBottom + 8),
      10,
      AppColors.muted,
    );
    _drawLabel(
    ...
    _drawLabel(
      canvas,
      '파워 스펙트럼',
      Offset(graphLeft + graphW / 2 - 18, graphTop - 1),
      9,
      AppColors.accent,
    );
    ...
    _drawLabel(
      canvas,
      '-ΔT',
      Offset(legendX - 2, legendTop + legendH + 1),
      8,
  ...
  @override
  bool shouldRepaint(covariant _CosmicMicrowaveBgScreenPainter oldDelegate) =>
      true;
}
