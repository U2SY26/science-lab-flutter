...
class _HubbleExpansionScreenPainter extends CustomPainter {
  ...
  void _drawExpansion(Canvas canvas, Size size, double h) {
    ...
    drawText(
      'H₀=${hubbleConst.toStringAsFixed(0)} km/s/Mpc',
      Offset(size.width - 120, 4),
    ...
    drawText(
      '허블 구',
      Offset(cx + hubbleSphereR.clamp(10.0, cx - 30) + 2, cy - 8),
  ...
  void _drawHubbleDiagram(Canvas canvas, Offset origin, double w, double h) {
    ...
    drawText(
      'v=H₀d',
      Offset(ox + plotW * 0.55, oy - plotH * 0.62),
      color: const Color(0xFFFF6B35),
      fs: 8,
    );
    ...
    if (selY >= origin.dy + padT) {
      ...
      drawText(
        'v=${selV.toStringAsFixed(0)}',
        Offset(selX + 4, selY - 12),
        color: const Color(0xFF64FF8C),
        fs: 8,
      );
    }
    ...
    drawText(
      't_universe≈${tAge.toStringAsFixed(1)} Gyr',
  ...
  @override
  bool shouldRepaint(covariant _HubbleExpansionScreenPainter oldDelegate) =>
      true;
}
