      ...
      body: SingleChildScrollView(
        ...
        child: SimulationContainer(
          ...
          simulation: SizedBox(
            height: 350,
            child: CustomPaint(
              painter: _KerrBlackHoleScreenPainter(time: _time, spin: _spin),
              size: Size.infinite,
            ),
          ),
          controls: Column(
...
class _KerrBlackHoleScreenPainter extends CustomPainter {
  ...
  _KerrBlackHoleScreenPainter({required this.time, required this.spin});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final ergoB =
        (1.0 + math.sqrt(1.0 - a * a * 0.0)) * scale; // polar (simplified)
    canvas.drawOval(
      Rect.fromCenter(
        center: Offset(cx, cy),
        width: ergoA * 2,
    ...
    _drawLabel(
      canvas,
      'a=${spin.toStringAsFixed(2)}',
      Offset(cx - 18, cy + rPlusR + 6),
      9,
      AppColors.muted,
    );
    ...
    for (int i = 0; i < 8; i++) {
      ...
      canvas.drawCircle(
        ...
        1.5 * (1 - t2 * 0.6),
        Paint()
          ..color = Color.fromARGB(((1 - t2) * 200).toInt(), 100, 255, 140),
      );
    }
    ...
    _drawLabel(
      canvas,
      'r+=${rPlus.toStringAsFixed(2)}M',
      Offset(cx + rPlusR + 3, cy + 4),
      9,
      AppColors.accent,
    );
    _drawLabel(
