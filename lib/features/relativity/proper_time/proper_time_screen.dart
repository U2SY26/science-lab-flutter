      ...
      body: SingleChildScrollView(
        ...
        child: SimulationContainer(
          ...
          simulation: SizedBox(
            height: 350,
            child: CustomPaint(
              painter: _ProperTimeScreenPainter(time: _time, beta: _beta),
              size: Size.infinite,
            ),
          ),
          controls: Column(
...
class _ProperTimeScreenPainter extends CustomPainter {
  ...
  _ProperTimeScreenPainter({required this.time, required this.beta});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawLine(
      Offset(ox, oy),
      Offset(dR, dT + (dB - dT) * (1 - (dW / 2) / dH)),
      lcP,
    );
    canvas.drawLine(
      Offset(ox, oy),
      Offset(dL, dT + (dB - dT) * (1 - (dW / 2) / dH)),
      lcP,
    );
    _lbl(
      canvas,
    ...
    _lbl(
      canvas,
      'A 나이: ${(coordTimeA / 20).toStringAsFixed(1)} yr',
      Offset(ox - 18, dT - 2),
      const Color(0xFF00D4FF),
      8,
    );
    _lbl(
      canvas,
      'B 나이: ${(properTimeB / 20).toStringAsFixed(1)} yr',
      Offset(ox + 50, dT - 2),
      const Color(0xFFFF6B35),
      8,
    );
    ...
    _lbl(
      canvas,
      'γ = ${gamma.toStringAsFixed(3)}',
      Offset(iL + iW / 2, gageT + 8),
    ...
    canvas.drawRRect(
      RRect.fromRectAndRadius(
        Rect.fromLTWH(bar1X, barT2, barW5, barH3),
    ...
    _lbl(
      canvas,
      'Δt',
      Offset(bar1X + barW5 / 2, barT2 + barH3 / 2),
    ...
    _lbl(
      canvas,
      'Δτ',
      Offset(bar2X + barW5 / 2, barBotY2 - propH / 2),
      const Color(0xFFE0F4FF),
      9,
    );
    ...
    canvas.drawLine(
      Offset(bar1X + barW5, barBotY2 - propH),
    ...
    _lbl(
      canvas,
      '÷γ',
      Offset(iL + iW / 2, barBotY2 - propH - 6),
      const Color(0xFF5A8A9A),
      8,
    );
    ...
    _lbl(
      canvas,
      'A (지구)',
      Offset(bar1X + barW5 / 2, barBotY2 + 8),
      const Color(0xFF00D4FF),
      8,
    );
    _lbl(
      canvas,
      'B (로켓)',
      Offset(bar2X + barW5 / 2, barBotY2 + 8),
      const Color(0xFFFF6B35),
      8,
    );
    ...
    _lbl(
      canvas,
      'β=${v.toStringAsFixed(2)}  γ=${gamma.toStringAsFixed(3)}',
      Offset(iL + iW / 2, h * 0.93),
      const Color(0xFF5A8A9A),
      8,
    );
  }
