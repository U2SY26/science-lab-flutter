...
class FriedmannEquationsScreen extends StatefulWidget {
  ...
  @override
  State<FriedmannEquationsScreen> createState() =>
      _FriedmannEquationsScreenState();
}
...
class _FriedmannEquationsScreenPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 1; i <= 4; i++) {
      ...
      drawText('$iτ', Offset(x - 4, originY + 4));
    }
    for (int i = 1; i <= 3; i++) {
    ...
    Offset toCanvas(double t, double a) =>
        Offset(originX + w * t / tMax, originY - h * a / aMax);
    ...
    drawText(
      'Ω=1 평탄',
      Offset(originX + w * 0.62, padT + h * 0.28),
    ...
    drawText(
      'Ω<1 열린',
      Offset(originX + w * 0.68, padT + h * 0.48),
      color: const Color(0xFF5A8A9A),
      fs: 8,
    );
    ...
    final curvLabel = curvatureK.abs() < 0.05
        ...
        : (curvatureK > 0 ? '열린' : '닫힌');
    drawText(
      'Ω_m=${omegaM.toStringAsFixed(2)}  Ω_Λ=${omegaL.toStringAsFixed(2)}  K:$curvLabel',
  ...
  @override
  bool shouldRepaint(covariant _FriedmannEquationsScreenPainter oldDelegate) =>
      true;
}
