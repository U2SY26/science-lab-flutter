...
class EquivalencePrincipleScreen extends StatefulWidget {
  ...
  @override
  State<EquivalencePrincipleScreen> createState() =>
      _EquivalencePrincipleScreenState();
}
              ...
              Container(
                ...
                child: Row(
                  children: [
                    ...
                    _V(
                      '차이',
                      (_acceleration - _gField).abs().toStringAsFixed(3),
                    ),
                  ],
                ),
              ),
...
class _EquivalencePrincipleScreenPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final ballY =
        elvT + 10 + (elvB - elvT - 30) * (ballPhase * ballPhase / 1.44);
    canvas.drawCircle(
    ...
    for (int k = 1; k <= 5; k++) {
      ...
      canvas.drawCircle(
        ...
        2.5,
        Paint()
          ..color = const Color(0xFFFF6B35).withValues(alpha: 0.3 - k * 0.04),
      );
    }
    ...
    _drawPhotonArrow(
      canvas,
      Offset(elvL + 8, elvT + 20),
      Offset(elvL + 8, elvT + 60),
    ...
    canvas.drawLine(
      Offset(arrX, rktB - 10),
      Offset(arrX, rktB - 10 - 30 * gNorm - 10),
      arrPaint,
    );
    canvas.drawLine(
      Offset(arrX, rktB - 10 - 30 * gNorm - 10),
      Offset(arrX - 5, rktB - 10 - 30 * gNorm),
      arrPaint,
    );
    canvas.drawLine(
      Offset(arrX, rktB - 10 - 30 * gNorm - 10),
    ...
    drawLabel(
      'a',
      Offset(arrX + 7, rktB - 30 * gNorm - 20),
      color: const Color(0xFFFF6B35),
      fs: 10,
    );
    ...
    canvas.drawCircle(
      Offset(
        rktCx - rktW * 0.1,
        rktT + 24 + (rktB - rktT - 40) * (ballPhase * ballPhase / 1.44),
      ),
      6,
      Paint()..color = const Color(0xFFFF6B35),
    );
  ...
  void _drawPhotonArrow(
    ...
    for (int i = 0; i < steps; i++) {
      ...
      canvas.drawLine(
        ...
        Offset(
          from.dx + dx * (i + 1) + math.cos((i + 1) * 1.2) * 3,
          from.dy + dy * (i + 1),
        ),
        Paint()
          ..color = c
  ...
  @override
  bool shouldRepaint(
    covariant _EquivalencePrincipleScreenPainter oldDelegate,
  ) => true;
}
