...
class _FrameDraggingScreenPainter extends CustomPainter {
  ...
  _FrameDraggingScreenPainter({required this.time, required this.spinParam});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final omega =
        a / (2.0 + 2.0 * math.sqrt(1.0 - a * a)); // frame-drag angular vel
    ...
    for (int ring = 1; ring <= gridRings; ring++) {
      ...
      for (int s = 0; s <= 64; s++) {
        ...
        final py =
            cy +
            r * 0.55 * math.sin(angle); // squashed to ellipse for perspective
        if (s == 0) {
          path.moveTo(px, py);
        } else {
          path.lineTo(px, py);
    ...
    canvas.drawLine(
      Offset(gyroX, gyroY),
      Offset(
        gyroX + 10 * math.cos(precessAngle),
        gyroY + 10 * math.sin(precessAngle),
      ),
      Paint()
    ...
    drawText(
      '자이로스코프',
      Offset(gyroX + 7, gyroY - 5),
      color: const Color(0xFF64FF8C),
      fs: 7,
    );
    drawText(
      '이벤트 호라이즌',
      Offset(cx - 38, cy - bhRy * 0.55 - 12),
      color: const Color(0xFF00D4FF),
      fs: 7,
    );
