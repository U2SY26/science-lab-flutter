...
class _BackpropagationScreenPainter extends CustomPainter {
  ...
  _BackpropagationScreenPainter({required this.time, required this.layers});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int l = 0; l < usedArch.length; l++) {
      ...
      nodePos.add(
        List.generate(n, (i) => Offset(layerXs[l], spacing * (i + 1) + 4)),
      );
    }
    ...
    for (int l = 0; l < usedArch.length; l++) {
      ...
      for (int n = 0; n < nodePos[l].length; n++) {
        ...
        for (final r in [18.0, 12.0, 8.0]) {
          canvas.drawCircle(
            ...
            r,
            Paint()
              ..color = _cyan.withValues(alpha: (18 - r) / 18 * 0.12 * gMag),
          );
        }
      ...
      _drawText(
        canvas,
        l < labels.length ? labels[l] : 'H$l',
        Offset(layerXs[l], netH + 2),
        9,
  ...
  void _drawLossCurve(Canvas canvas, Size size, double top, double h) {
    ...
    canvas.drawLine(
      Offset(left + 2, top + 2),
      Offset(left + 2, top + h - 2),
      axisPaint,
    );
    canvas.drawLine(
      Offset(left + 2, top + h - 2),
      Offset(right - 2, top + h - 2),
      axisPaint,
    );
    ...
    // Current loss dot
    final curLoss =
        math.exp(-math.min(time, 8.0) * 0.4) *
        (0.85 + 0.1 * math.sin(math.min(time, 8.0) * 3.1));
    final dotX = right - 6;
    final dotY = top + (h - 4) * (1 - curLoss.clamp(0.0, 1.0));
    canvas.drawCircle(Offset(dotX, dotY), 3, Paint()..color = _orange);
    canvas.drawCircle(
      Offset(dotX, dotY),
      6,
  ...
  @override
  bool shouldRepaint(covariant _BackpropagationScreenPainter oldDelegate) =>
      true;
}
