...
class _ConfusionMatrixScreenPainter extends CustomPainter {
  ...
  _ConfusionMatrixScreenPainter({required this.time, required this.threshold});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    const topPad = 22.0;
    const matPad = 8.0;
    final matSize = math.min(
      size.width - labelW * 2 - matPad * 2,
    ...
    for (int c = 0; c < _n; c++) {
      ...
      tp.paint(
        canvas,
        Offset(matLeft + c * cellSize + (cellSize - tp.width) / 2, topPad),
      );
    }
    ...
    for (int r = 0; r < _n; r++) {
      for (int c = 0; c < _n; c++) {
        ...
        vtp.paint(
          canvas,
          Offset(
            rect.left + (rect.width - vtp.width) / 2,
    ...
    for (int i = 0; i < metrics.length; i++) {
      ...
      canvas.drawRRect(
        RRect.fromRectAndRadius(
          Rect.fromLTWH(bx, barTop, barW, maxBarH),
      ...
      ltp.paint(
        canvas,
        Offset(bx + (barW - ltp.width) / 2, barTop + maxBarH + 2),
      );
      ...
      vtp.paint(
        canvas,
        Offset(bx + (barW - vtp.width) / 2, barTop + maxBarH - filledH - 12),
      );
    }
  }

  @override
  bool shouldRepaint(covariant _ConfusionMatrixScreenPainter oldDelegate) =>
      true;
}
