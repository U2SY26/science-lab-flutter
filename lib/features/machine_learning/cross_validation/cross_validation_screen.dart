...
class _CrossValidationScreenPainter extends CustomPainter {
  ...
  _CrossValidationScreenPainter({required this.time, required this.kFolds});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final activeFold = ((time % cycleLen) / cycleLen * k).floor().clamp(
      0,
      k - 1,
    );
    ...
    final scores = List.generate(
      k,
      (i) =>
          (baseScores[i] + 0.03 * math.sin(time * 0.6 + i)).clamp(0.55, 0.99),
    );
    ...
    for (int i = 0; i < k; i++) {
      ...
      ltp.paint(
        canvas,
        Offset(
          bx + (blockW - ltp.width) / 2,
          stripTop + (stripH - ltp.height) / 2,
        ),
      );
    }
    ...
    _drawChip(
      canvas,
      Offset(stripPadX, stripTop + stripH + 6),
      'Train',
      const Color(0xFF00D4FF),
    );
    _drawChip(
      canvas,
      Offset(stripPadX + 58, stripTop + stripH + 6),
      'Val',
        ...
        Paint()
          ..shader = LinearGradient(
            ...
            colors: [
              ...
              color.withValues(alpha: isCur ? 1.0 : 0.75),
            ],
    ...
    fstp.paint(
      canvas,
      Offset(stripPadX + stripW - fstp.width, barAreaTop - 12),
    );
    ...
    final axPaint = Paint()
      ...
      ..strokeWidth = 1;
    canvas.drawLine(
      Offset(stripPadX, chartTop),
      Offset(stripPadX, chartTop + chartH),
      axPaint,
    );
    canvas.drawLine(
      Offset(stripPadX, chartTop + chartH),
      Offset(stripPadX + chartW, chartTop + chartH),
      axPaint,
    );

    final meanScore = scores.reduce((a, b) => a + b) / k;
    final std = math.sqrt(
      scores.map((s) => math.pow(s - meanScore, 2)).reduce((a, b) => a + b) / k,
    );

    // Training score (approaches 1 as k increases) & val score = mean
    final trainScores = List.generate(
      k,
      (i) =>
          (0.92 - 0.01 * i + 0.01 * math.sin(time * 0.5 + i)).clamp(0.7, 0.99),
    );
    ...
    for (int i = 0; i < k; i++) {
      ...
      final yTop =
          chartTop +
          chartH -
          (((meanScore + std) - 0.5) / 0.5 * chartH).clamp(0.0, chartH);
      final yBot =
          chartTop +
          chartH -
          (((meanScore - std) - 0.5) / 0.5 * chartH).clamp(0.0, chartH);
      if (i == 0) {
        bandPath.moveTo(xf, yTop);
        bandPathBottom.moveTo(xf, yBot);
      } else {
        bandPath.lineTo(xf, yTop);
        bandPathBottom.lineTo(xf, yBot);
      }
    }
    for (int i = k - 1; i >= 0; i--) {
      final xf = stripPadX + i / (k - 1).clamp(1, 100) * chartW;
    ...
    canvas.drawPath(
      bandPath,
      Paint()..color = const Color(0xFF00D4FF).withValues(alpha: 0.1),
    );
    ...
    _drawLineCurve(
      ...
      List.generate(k, (i) {
        ...
        final yf =
            chartTop +
            chartH -
            ((valScores[i] - 0.5) / 0.5 * chartH).clamp(0.0, chartH);
        return Offset(xf, yf);
      }),
      const Color(0xFF00D4FF),
      2.0,
    ...
    _drawLineCurve(
      ...
      List.generate(k, (i) {
        ...
        final yf =
            chartTop +
            chartH -
            ((trainScores[i] - 0.5) / 0.5 * chartH).clamp(0.0, chartH);
        return Offset(xf, yf);
      }),
    ...
    _drawChip(
      canvas,
      Offset(stripPadX, chartTop - 14),
      'Train Acc',
      const Color(0xFF64FF8C),
    );
    _drawChip(
      canvas,
      Offset(stripPadX + 72, chartTop - 14),
      'Val Acc',
      const Color(0xFF00D4FF),
    );
  ...
  @override
  bool shouldRepaint(covariant _CrossValidationScreenPainter oldDelegate) =>
      true;
}
