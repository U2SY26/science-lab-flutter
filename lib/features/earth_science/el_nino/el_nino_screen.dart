      ...
      body: SingleChildScrollView(
        ...
        child: SimulationContainer(
          ...
          simulation: SizedBox(
            height: 350,
            child: CustomPaint(
              painter: _ElNinoScreenPainter(time: _time, soi: _soi),
              size: Size.infinite,
            ),
          ),
          controls: Column(
...
class _ElNinoScreenPainter extends CustomPainter {
  ...
  _ElNinoScreenPainter({required this.time, required this.soi});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i <= 60; i++) {
      ...
      if (i == 0) {
        thermoPath.moveTo(x, y);
      } else {
        thermoPath.lineTo(x, y);
      }
    }
    ...
    final windDir = soi >= 0
        ? -1.0
        : 1.0; // normal/la nina: west→east, el nino: reversed
    ...
    if (graphH > 20) {
      ...
      drawLabel(
        'SOI: ${soi.toStringAsFixed(1)}  ($phase)',
        w / 2,
        graphTop + graphH * 0.15,
        barColor,
        9,
        fw: FontWeight.w600,
      );
