...
class _JetStreamScreenPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final latPaint = Paint()
      ..color = const Color(0xFF1A3040).withValues(alpha: 0.7)
      ..strokeWidth = 0.6;
    ...
    double jetY(double x, double baseLatFrac, double waveNum, double phaseOff) {
      ...
      return h * baseLatFrac +
          h *
              0.065 *
              amplitude *
              math.sin(waveNum * x / w * math.pi * 2 - phase);
    }
    ...
    for (final jet in jets) {
      ...
      for (int px = 0; px <= w.toInt(); px += 2) {
        ...
        if (first) {
          glowPath.moveTo(px.toDouble(), y);
          first = false;
        } else {
          glowPath.lineTo(px.toDouble(), y);
        }
      }
      canvas.drawPath(
        glowPath,
      ...
      for (int px = 0; px <= w.toInt(); px += 2) {
        final y = jetY(px.toDouble(), baseLatFrac, waveNum, phaseOff);
        if (first) {
          mainPath.moveTo(px.toDouble(), y);
          first = false;
        } else {
          mainPath.lineTo(px.toDouble(), y);
        }
      }
      canvas.drawPath(
        mainPath,
      ...
      for (int p = 0; p < 8; p++) {
        final tFrac =
            ((p / 8.0) + time * speedFactor * 0.5 + phaseOff * 0.1) % 1.0;
        final px = tFrac * w;
      ...
      _drawLabel(
        canvas,
        label,
        Offset(w * 0.72, labelY - 10),
        color: jetColor.withValues(alpha: 0.9),
        fontSize: 8,
      );
    }
    ...
    _drawLabel(
      canvas,
      '진폭 ${amplitude.toStringAsFixed(2)}  |  속도 ${speed.toInt()} km/h',
      Offset(w * 0.5, h - 10),
      color: const Color(0xFF3A5A6A),
      fontSize: 8,
    );
