...
class _WeatherFrontsScreenPainter extends CustomPainter {
  ...
  void _drawArrow(Canvas canvas, Offset from, Offset to, Paint paint) {
    ...
    final arrowSize = 7.0;
    final p1 = Offset(
      to.dx - arrowSize * (ux - uy * 0.5),
      to.dy - arrowSize * (uy + ux * 0.5),
    );
    final p2 = Offset(
      to.dx - arrowSize * (ux + uy * 0.5),
      to.dy - arrowSize * (uy - ux * 0.5),
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (fType == 0) {
      ...
      _drawLabel(
        canvas,
        '한랭전선',
        Offset(frontStartX + w * 0.17, frontY - 18),
        color: const Color(0xFF4488FF),
        fontSize: 10,
      );
    } else if (fType == 1) {
      ...
      for (double x = frontStartX + 9; x < frontStartX + w * 0.35; x += 18) {
        final semiRect = Rect.fromCenter(
          center: Offset(x, frontY),
          width: 14,
          height: 14,
        );
    ...
    final cloudPaint = Paint()
      ..color = const Color(0xFFE0F4FF).withValues(alpha: 0.15)
      ..style = PaintingStyle.fill;
    ...
    for (int i = 0; i < 6; i++) {
      ...
      final cx2 =
          (baseX + time * speed * 25 * (0.5 + rng.nextDouble() * 0.5)) % w;
      final r = 18.0 + rng.nextDouble() * 16;
      canvas.drawOval(
        Rect.fromCenter(center: Offset(cx2, baseY), width: r * 2.2, height: r),
        cloudPaint,
      );
      canvas.drawOval(
        Rect.fromCenter(
          center: Offset(cx2 + r * 0.5, baseY - r * 0.3),
          width: r * 1.4,
          height: r * 0.8,
        ),
        cloudPaint,
