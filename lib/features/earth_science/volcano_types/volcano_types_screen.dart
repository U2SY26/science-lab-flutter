...
class _VolcanoTypesScreenPainter extends CustomPainter {
      ...
      Paint()
        ..shader =
            LinearGradient(
              ...
              Rect.fromLTWH(cx - baseW, groundY - peakH, baseW * 2, peakH),
            ),
    );
    canvas.drawPath(
      path,
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int p = 0; p < particleCount; p++) {
      final tFrac =
          ((time * (0.6 + rng.nextDouble() * 0.6) +
              p / particleCount.toDouble())) %
          1.0;
      final angle =
          -math.pi / 2 + (rng.nextDouble() - 0.5) * (0.8 + intensityFrac * 1.2);
      final launchSpeed = 60 + intensityFrac * 120;
      final px = vcx + math.cos(angle) * launchSpeed * tFrac;
      final py =
          peakY +
          math.sin(angle) * launchSpeed * tFrac +
          0.5 * 200 * tFrac * tFrac; // gravity
    ...
    canvas.drawCircle(
      ...
      Paint()
        ..color = const Color(
          0xFFFF6600,
        ).withValues(alpha: 0.6 + 0.3 * math.sin(time * 4)),
    );
    ...
    _drawLabel(
      canvas,
      '마그마 챔버',
      Offset(vcx + baseW * 0.5, groundY + h * 0.1),
