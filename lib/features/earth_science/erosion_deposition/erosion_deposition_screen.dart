...
class ErosionDepositionScreen extends StatefulWidget {
  ...
  @override
  State<ErosionDepositionScreen> createState() =>
      _ErosionDepositionScreenState();
}
...
class _ErosionDepositionScreenState extends State<ErosionDepositionScreen>
  ...
  void _update() {
    ...
    setState(() {
      ...
      _erosionRate =
          0.01 *
          math.pow(_flowRate, 0.6).toDouble() *
          math.pow(_slope / 45.0, 1.5).toDouble();
...
class _ErosionDepositionScreenPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawRect(
      ...
      Paint()..color = const Color(0xFF08100A),
    );
    ...
    final mountainPath = Path()
      ...
      ..quadraticBezierTo(
        ...
        mountainPeakY,
        w * 0.12,
        mountainPeakY + h * 0.04,
      )
      ..quadraticBezierTo(
        w * 0.2,
        mountainPeakY + h * 0.08,
        mountainEndX,
        groundY,
      )
      ..lineTo(0, groundY)
    ...
    for (int px = 0; px <= 200; px++) {
      ...
      if (t < 0.32) {
        ...
        y =
            mountainPeakY +
            h * 0.06 +
            (groundY - (mountainPeakY + h * 0.06)) * math.pow(lerpT, 1.8);
        ...
        y =
            groundY -
            h * 0.005 +
            math.sin(lerpT * math.pi * 3 - time * flowFrac * 0.8) * meanderAmp;
      } else {
        // Lower delta: fan out
        ...
        y =
            ...
            h * 0.005 -
            lerpT * h * 0.02 +
            math.sin(lerpT * math.pi * 2) * h * 0.015;
      }
    ...
    for (int layer = 2; layer >= 0; layer--) {
      ...
      for (int i = 0; i < riverPoints.length; i++) {
        ...
        if (i == 0) {
          riverPath.moveTo(top.dx, top.dy);
        } else {
          riverPath.lineTo(top.dx, top.dy);
        }
      }
      ...
      canvas.drawPath(
        riverPath,
        Paint()..color = speedColor.withValues(alpha: alphaVal),
      );
    }
    ...
    for (int p = 0; p < particleCount; p++) {
      ...
      final idx = (tBase * (riverPoints.length - 1)).toInt().clamp(
        0,
      ...
      canvas.drawCircle(
        pos +
            Offset((rng.nextDouble() - 0.5) * 6, (rng.nextDouble() - 0.5) * 4),
        particleR,
    ...
    _drawLabel(
      canvas,
      'V자 계곡\n(침식)',
      Offset(w * 0.10, mountainPeakY - 10),
    ...
    _drawLabel(
      canvas,
      '유량 ${flowRate.toStringAsFixed(1)} m³/s',
      Offset(w * 0.15, h * 0.96),
      color: const Color(0xFF00AADD),
      fontSize: 8,
    );
    _drawLabel(
      canvas,
      '경사 ${slope.toStringAsFixed(0)}°',
      Offset(w * 0.85, h * 0.96),
      color: const Color(0xFFFF6B35),
      fontSize: 8,
    );
  }
  ...
  @override
  bool shouldRepaint(covariant _ErosionDepositionScreenPainter oldDelegate) =>
      true;
}
