...
class _QuantumWellScreenPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final wellFrac = (wellWidth / 5.0).clamp(
      0.05,
      1.0,
    ...
    final e1 = (3.81e-38 / (9.11e-31 * L * L) / 1.6e-19).clamp(
      0.001,
      wellDepth * 2,
    );
    ...
    canvas.drawLine(
      Offset(wellLeft, wellBottom),
      Offset(wellLeft, wallTop),
      wallPaint,
    );
    // Right wall
    canvas.drawLine(
      Offset(wellRight, wellBottom),
      Offset(wellRight, wallTop),
      wallPaint,
    );
    // Bottom
    canvas.drawLine(
    ...
    _label(
      canvas,
      'V₀=${wellDepth.toStringAsFixed(1)}eV',
      Offset(padL, wallTop - 12),
      color: const Color(0xFF5A8A9A),
    );
    ...
    for (int n = 1; n <= maxN; n++) {
      ...
      _label(
        canvas,
        'n=$n  E=${en.toStringAsFixed(2)}eV',
        Offset(wellRight + 4, ey - 5),
        color: color,
        fontSize: 8,
      );
    ...
    _label(
      canvas,
      'x (위치)',
      Offset(padL + plotW * 0.4, h - padB + 6),
      color: const Color(0xFF5A8A9A),
      fontSize: 9,
    );
    ...
    _label(
      canvas,
      'L=${wellWidth.toStringAsFixed(1)}nm',
    ...
    _label(
      canvas,
      '양자 우물 (L=${wellWidth.toStringAsFixed(1)}nm, V₀=${wellDepth.toStringAsFixed(1)}eV)',
      Offset(padL, 2),
