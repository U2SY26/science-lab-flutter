...
class _GroverAlgorithmScreenState extends State<GroverAlgorithmScreen>
  ...
  void _update() {
    ...
    setState(() {
      ...
      _targetProb = math
          .pow(math.sin((2 * _iterations.toInt() + 1) * theta), 2)
          .toDouble();
...
class _GroverAlgorithmScreenPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawLine(
      Offset(chartL, baselineY),
      Offset(chartR, baselineY),
      axisP,
    );
    // Y axis
    ...
    _lbl(
      canvas,
      '진폭',
      Offset(chartL - 14, (chartTop + baselineY) / 2),
      const Color(0xFF5A8A9A),
      8,
    );
    ...
    _lbl(
      canvas,
      '타깃 #$targetIdx',
      Offset(targetBx, targetBarTop - 10),
    ...
    while (dx2 < chartR) {
      canvas.drawLine(
        Offset(dx2, avgY),
        Offset(math.min(dx2 + 5, chartR), avgY),
        Paint()
    ...
    _lbl(
      canvas,
      '반복 횟수 vs 성공 확률',
      Offset(graphL + graphW / 2, graphTop - 7),
    ...
    for (int k = 0; k <= maxIter2; k++) {
      final px = graphL + (k / maxIter2) * graphW;
      final prob = math
          .pow(math.sin((2 * k + 1) * theta), 2)
          .toDouble()
          .clamp(0.0, 1.0);
      final py = graphBot - prob * (graphH - 4);
      if (k == 0) {
        quantPath.moveTo(px, py);
      } else {
        quantPath.lineTo(px, py);
      }
    }
    canvas.drawPath(
      quantPath,
    ...
    _lbl(
      canvas,
      '─ 양자 O(√N)',
      Offset(graphL + graphW * 0.62, graphTop + 8),
      const Color(0xFF00D4FF),
      8,
    );
    _lbl(
      canvas,
      '─ 고전 O(N)',
      Offset(graphL + graphW * 0.62, graphTop + 18),
    ...
    _lbl(
      canvas,
      '최적 반복: $optIter ≈ π/4·√N',
      Offset(graphL + graphW * 0.28, graphBot + 11),
      const Color(0xFF5A8A9A),
      8,
    );
  }
  ...
  @override
  bool shouldRepaint(covariant _GroverAlgorithmScreenPainter oldDelegate) =>
      true;
}
