...
class QuantumTeleportationScreen extends StatefulWidget {
  ...
  @override
  State<QuantumTeleportationScreen> createState() =>
      _QuantumTeleportationScreenState();
}
...
class _QuantumTeleportationScreenPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < 3; i++) {
      final lp = Paint()
        ..color = lineColors[i].withValues(alpha: 0.4)
        ..strokeWidth = 1.5
        ..style = PaintingStyle.stroke;
    ...
    final eprRect = Rect.fromCenter(
      center: Offset(eprX, (yLines[1] + yLines[2]) / 2),
      width: 34,
      height: 44,
    );
    canvas.drawRRect(
      RRect.fromRectAndRadius(eprRect, const Radius.circular(4)),
    ...
    _drawLabel(
      canvas,
      'Bell',
      Offset(eprX, (yLines[1] + yLines[2]) / 2 - 5),
      const Color(0xFF64FF8C),
      8,
    );
    _drawLabel(
    ...
    for (double dx = 0; dx <= 1.0; dx += 0.05) {
      ...
      final yi1 =
          yLines[1] + waveAmp * math.sin(dx * math.pi * waveFreq + time * 3);
      if (dx == 0) {
        wavePath.moveTo(xi, yi1);
      } else {
        wavePath.lineTo(xi, yi1);
      }
    }
    canvas.drawPath(
      wavePath,
      Paint()
        ..color = const Color(0xFF64FF8C).withValues(alpha: 0.35)
        ..strokeWidth = 1
        ..style = PaintingStyle.stroke,
    );
    ...
    canvas.drawLine(
      Offset(cnotX, yLines[0] + 5),
      Offset(cnotX, yLines[1] - 10),
      Paint()
        ..color = const Color(0xFF00D4FF)
        ..strokeWidth = 1.5,
    );
    ...
    for (int qi = 0; qi < 2; qi++) {
      final mRect = Rect.fromCenter(
        center: Offset(mX0, yLines[qi]),
        width: 22,
        height: 18,
      );
      canvas.drawRRect(
        RRect.fromRectAndRadius(mRect, const Radius.circular(3)),
        Paint()..color = const Color(0xFF1A3040),
      );
      canvas.drawRRect(
        RRect.fromRectAndRadius(mRect, const Radius.circular(3)),
      ...
      canvas.drawLine(
        Offset(mX0, yLines[qi] + 2),
        Offset(mX0 + 5, yLines[qi] - 3),
        Paint()
          ..color = lineColors[qi]
          ..strokeWidth = 1,
      );
    ...
    while (cx0 < bobCorrX - 5) {
      canvas.drawLine(
        Offset(cx0, yLines[0]),
        Offset(math.min(cx0 + 6, bobCorrX), yLines[0]),
        dashedClassP,
      );
      canvas.drawLine(
        Offset(cx0, yLines[1]),
        Offset(math.min(cx0 + 6, bobCorrX), yLines[1]),
        dashedClassP,
      );
      cx0 += 10;
    }
    _drawLabel(
      canvas,
      '고전비트',
      Offset((classX + bobCorrX) / 2, yLines[0] - 10),
      const Color(0xFFFF6B35),
      8,
    );
    ...
    _drawBlochSphere(
      canvas,
      Offset(bobBlochX, blochY),
      blochR,
      theta,
      phi,
      const Color(0xFFFF6B35).withValues(alpha: bobAlpha),
      '밥 |ψ⟩ 복원',
    );
    ...
    final arrowP = Paint()
      ...
      ..style = PaintingStyle.stroke;
    canvas.drawLine(
      Offset(aliceBlochX + blochR + 4, blochY),
      Offset(bobBlochX - blochR - 4, blochY),
      arrowP,
    );
    canvas.drawLine(
      Offset(bobBlochX - blochR - 10, blochY - 4),
      Offset(bobBlochX - blochR - 4, blochY),
      arrowP,
    );
    canvas.drawLine(
    ...
    _drawLabel(
      canvas,
      '텔레포테이션',
      Offset((aliceBlochX + bobBlochX) / 2, blochY - blochR - 6),
      arrowColor,
      8,
    );
  }
  ...
  @override
  bool shouldRepaint(
    covariant _QuantumTeleportationScreenPainter oldDelegate,
  ) => true;
}
